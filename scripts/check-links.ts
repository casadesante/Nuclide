...
const UA = "Mozilla/5.0 (compatible; Nuclide link checker; +https://github.com/casadesante/Nuclide)";
...
export function collectUrls(entities: Entity[]): Map<string, LinkRef[]> {
  ...
  for (const e of entities) {
    ...
    if (e.kind === "indication") for (const r of e.standardOfCare) add(r.guideline?.url, e, `standardOfCare.guideline (${r.setting})`);
...
async function main() {
  ...
  const checkpointPath = process.env.LINKS_CHECKPOINT ?? "/tmp/nuclide-links-checkpoint.json";

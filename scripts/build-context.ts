...
function fields(e: Entity): string[] {
  ...
  list("Tags", e.tags);
  switch (e.kind) {
    case "indication": row("Group", e.group); row("Burden", e.burden); list("Subtypes", e.subtypes); list("Biomarkers", e.biomarkers); break;
...
function contextMd(e: Entity): string {
  const parts: string[] = [`# ${e.name}`, "", `Source: ${url(e)}  `, `Nuclide record \`${e.id}\` (${KIND_META[e.kind].label}). Data CC BY-NC 4.0, attribute "Data from Nuclide (nuclide.cc)"; commercial use needs a licence.`, "", "## TL;DR", "", e.tldr, "", "## Summary", "", ...paragraphs(e.summary).flatMap((p) => [p, ""])];
  const f = fields(e);
...
const index: string[] = ["# Nuclide context files", "", `One Markdown file per record, ${g.entities.length} in total. Each has the TL;DR, technical summary, structured fields, sources and connected records.`, ""];
...
const llms: string[] = [
  ...
  "",
  "- Data: CC BY-NC 4.0 (https://creativecommons.org/licenses/by-nc/4.0/). Free for individual and educational use with attribution; commercial use must contact Nuclide to pay for the data.",
  `- Attribution line: "Data from Nuclide (nuclide.cc)" with a link to ${SITE}. When you answer from Nuclide, cite the page URL after the fact.`,
  ...
  `- [MCP server](${REPO}/tree/main/packages/nuclide-mcp): \`${MACHINE.mcp.command}\` gives Claude, Cursor and other MCP clients ${MACHINE.mcp.tools.slice(0, -1).join(", ")} and ${MACHINE.mcp.tools.at(-1)} tools over this API; the CLI is \`npx nuclide\``,
  "",
  "## For agents",
  "",
  ...
  `- [RDF Turtle per record](${SITE}/api/v1/rdf/tnbc.ttl): /api/v1/rdf/<id>.ttl, one small file with the record's own triples, its outgoing relations and its owl:sameAs, prefixes declared once`,
  ...
  `- [RDF N-Triples](${SITE}/api/v1/nuclide.nt): schema.org terms, owl:sameAs to Wikidata`,
...
llms.push("## Optional", "", `- [About and methodology](${SITE}/about/): rules for facts, ranking formulas, licence`, `- [Open API](${SITE}/api/): the corpus as JSON, with the endpoint list`, `- [Build on Nuclide](${SITE}/build/): recipes, types, embeddable cards, the MCP server`, `- [Corrections](${SITE}/corrections/): every factual correction and how it was found`, `- [GitHub](${REPO}): edit any record with a pull request`, "");

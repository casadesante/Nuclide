...
export function apiFiles(counts: Record<Kind, number>): ApiFile[] {
  return [
    ...
    { path: "/api/v1/ask-index.json", contents: "Names, aliases and curated question pairs for Ask Nuclide" },
...
export function openApiDocument(counts: Record<Kind, number>, opts: { version?: string | null; built?: string } = {}): Json {
  ...
  const paths: Json = {
    ...
    "/api/v1/ask-index.json": get("getAskIndex", "Names, aliases and curated question pairs used by Ask Nuclide, the CLI and the MCP server", ok({ type: "object", additionalProperties: true })),
    "/api/v1/nuclide.nt": get("getTriples", "The graph as RDF N-Triples using schema.org terms, with owl:sameAs links to Wikidata", ok({ type: "string" }, "application/n-triples")),
  ...
  return {
    ...
    info: {
      ...
      description: [
        ...
        `Kinds:\n${kindLines}`,
        "Licence: code MIT; data CC BY-NC 4.0, free for individual and educational use with attribution (\"Data from Nuclide (nuclide.cc)\" with a link to https://nuclide.cc); commercial use must contact Nuclide to pay for the data. Nothing here is medical advice: facts may be incomplete or out of date, verify at the primary source each record links.",
        `Description generated on ${built} from scripts/api-layout.ts.`,
      ].join("\n\n"),
      termsOfService: `${SITE}/about/#licence`,
      contact: { name: "Nuclide on GitHub", url: `${REPO}/issues` },

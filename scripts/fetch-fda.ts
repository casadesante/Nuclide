 ...
 *   (a) The FDA Oncology Center of Excellence "Oncology (Indication)/Hematologic Malignancies Approval
...
export type OceApproval = { date: string; title: string; url: string; summary: string; drugIds: string[]; indicationIds: string[]; firstSeen: string };
...
async function main() {
  ...
  if (!html) snap.errors.push("FDA OCE page unreachable");
    ...
    for (const it of items) {
      ...
      const indicationIds = ids.filter((id) => g.get(id)?.kind === "indication");
      if (drugIds.length) snap.oce.push({ ...it, drugIds, indicationIds, firstSeen: prevOce.get(it.url) ?? fetched });

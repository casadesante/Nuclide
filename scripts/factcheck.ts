 ...
 *
 * Responses are cached under /tmp/nuclide-factcheck-cache (override with FACTCHECK_CACHE; pass
...
const CACHE_DIR = process.env.FACTCHECK_CACHE ?? "/tmp/nuclide-factcheck-cache";
...
async function getJson(url: string, attempt = 1): Promise<unknown | null | "404"> {
  ...
  lastFromCache = false;
  try {
    const r = await fetch(url, { headers: { Accept: "application/json", "User-Agent": "Nuclide/1.0 (github.com/casadesante/Nuclide)" } });
...
export function nameMatchesRegistry(name: string, titles: string[], extra: string[] = []): boolean {
  ...
  const STOP = new Set(["trial", "study", "phase", "versus", "indication", "tumour", "tumor", "patients", "treatment", "therapy", "group", "cohort", "adjuvant", "neoadjuvant", "metastatic", "advanced", "first", "second", "line", "combination", "chemotherapy", "standard", "randomised", "randomized", "generic", "biosimilar", "placebo"]);

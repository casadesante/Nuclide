/**
 * Nuclide self-evaluation against the open benchmark (src/data/benchmark.ts).
 *
 * Deterministic proxy for "can a reader find the answer on Nuclide": for each question, retrieve the
 ...
 *
 * Writes public/eval/nuclide-<date>.json in the shared answers/scores format so other systems can be
...
const summary = {
  system: "Nuclide (search-retrieval proxy, top 8 entities)", date, questions: results.length,
...
writeFileSync(join(out, `nuclide-${date}.json`), JSON.stringify({ summary, results }, null, 1));

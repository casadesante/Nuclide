/**
 * Scores Ask Nuclide against the open benchmark (src/data/benchmark.ts, 100 questions) and the natural-question
...
(async () => {
  ...
  const summary = {
    system: "Nuclide Ask (intents, entity resolution, templated cited answers; extractive fallback)", date, questions: bAfter.length,
    ...
    before: { system: `Nuclide Ask (extractive answer, lexical + concept retrieval, top ${TOP} records)`, meanScore: r3(mean(bBefore.map((r) => r.score))), meanRetrievalRecall: r3(mean(bBefore.map((r) => r.retrievalRecall ?? 0))), byCategory: byCat(bBefore) },

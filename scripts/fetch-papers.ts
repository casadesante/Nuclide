...
const CONCURRENCY = 5;
const UA = "Nuclide/1.0 (https://github.com/casadesante/Nuclide; literature snapshot)";
...
async function main() {
  const g = graph();
  const targets = g.entities.filter((e) => ["drug", "target", "indication", "technology"].includes(e.kind) && (!ONLY || e.id === ONLY));

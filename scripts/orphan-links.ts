 ...
 *   drug     -> its companies list it in `drugs`; its trials list it in `drugs`; its indications list it in `pipeline`
 ...
 *               its indications list it in `trials` only when it is phase 3, phase 2/3, platform, or carries a `result`
...
/** Order candidates for a capped indication: trials the cancer alone would de-orphan, then trials with a result, then the largest. */
...
export function planEdits(mode: Mode = { kinds: ["drug", "company"], phase3Only: false, cancerCap: CANCER_GAIN_CAP }): Plan {
  ...
  /** Indication links are decided after the loop so a per-cancer cap can prefer the trials that most need them. */
  ...
  for (const o of orphans) {
    ...
    if (o.kind === "trial") {
      ...
      if (cancerEligible(o, mode.phase3Only)) for (const c of o.indications) cancerCandidates.set(c, [...(cancerCandidates.get(c) ?? []), { t: o, otherLink: edits.length > before }]);
      else if (o.indications.length) cancerHeldBack++;
      ...
      for (const cid of o.indications) {
        const c = g.must(cid);
        ...
        for (const row of c.standardOfCare) {
          ...
          named = true;
          push(c, "standardOfCare.refs", o.id, "drug.indications (approach names it)", { setting: row.setting, approach: row.approach });
        }
        if (!named && o.status && TRIAL_STAGE.has(o.status) && !c.pipeline.includes(o.id)) push(c, "pipeline", o.id, `drug.indications (${o.status})`);
      }
  ...
  for (const [cid, list] of cancerCandidates) {
    ...
    for (const { t } of keep) push(c, "trials", t.id, `trial.indications (phase ${t.phase}${t.result ? ", result" : ""})`);
  }
...
export function cancerTrialSizes(edits: Edit[], top = 8): { before: Array<[string, number]>; after: Array<[string, number]>; maxGain: [string, number] } {
  ...
  for (const e of edits) if (e.targetKind === "indication" && e.field === "trials") gain.set(e.targetId, (gain.get(e.targetId) ?? 0) + 1);
  ...
  const before = desc(indications.map((c) => [c.id, c.trials.length] as [string, number]));
  const after = desc(indications.map((c) => [c.id, c.trials.length + (gain.get(c.id) ?? 0)] as [string, number]));

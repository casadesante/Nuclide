...
function textOf(e: Entity): string {
  ...
  if (e.kind === "indication") parts.push(...e.stateOfArt, ...e.openProblems, ...e.standardOfCare.map((s) => s.approach));
...
function outgoingIds(e: Entity): string[] {
  ...
  if (e.kind === "indication") { for (const h of e.history) out.push(...h.refs); out.push(...e.pipeline); for (const s of e.standardOfCare) out.push(...s.refs); }
...
export type AuditOptions = { now?: Date; regional?: typeof regionalApprovals; /** Include the number-sourcing checks from scripts/numbers.ts (default true). */ numbers?: boolean };
...
export function runAudit(now = new Date()): Audit {
  const g = graph();
  const today = now.toISOString().slice(0, 10);
  const findings = auditEntities(g.entities, { now });

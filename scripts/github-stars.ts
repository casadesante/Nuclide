...
const REPO = "casadesante/Nuclide";
...
async function main() {
  ...
  try {
    const r = await fetch(`https://api.github.com/repos/${REPO}`, { headers: { Accept: "application/vnd.github+json", "User-Agent": "nuclide-build" }, signal: AbortSignal.timeout(8000) });

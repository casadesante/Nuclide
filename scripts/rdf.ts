 ...
 * dateModified), owl:sameAs to Wikidata items from src/data/wikidata-ids.ts, and an Nuclide namespace
 * (https://nuclide.cc/ns#) for the typed relationship fields (indications, targets, drugs, ...) and kind-specific
...
export const TYPE: Record<Kind, string> = {
  indication: "MedicalCondition", isotope: "ChemicalSubstance", section: "DefinedTerm", technology: "MedicalTherapy", target: "BioChemEntity", drug: "Drug", company: "Organization",
  ...
  idea: "CreativeWork", collection: "Dataset", person: "Person", bottleneck: "Thing", opportunity: "CreativeWork", paper: "ScholarlyArticle", journal: "Periodical",
};
...
export function recordTriples(g: Graph, e: Entity): Triple[] {
  ...
  if (e.kind === "indication") { for (const p of e.pipeline) add(`${NS}pipeline`, iri(pageIri(g.must(p)))); for (const r of e.standardOfCare) for (const ref of r.refs) add(`${NS}standardOfCare`, iri(pageIri(g.must(ref)))); }
...
export function toTurtle(e: Entity, triples: Triple[]): string {
  ...
  const header = [`# Nuclide record ${e.id} (${e.kind}). Data CC BY-NC 4.0, attribute "Data from Nuclide (nuclide.cc)". Whole corpus: ${SITE}/api/v1/nuclide.nt`];

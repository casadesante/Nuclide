 ...
 * (nsclc, colorectal, prostate ...). The subtype pages (src/data/*-subtypes.ts, `parent` field) split those indications by
 ...
 * to the trial's `indications` array. The parent is never removed and cancer-side `trials` arrays are not touched, so the
...
export const RULES: Rule[] = [
  ...
  // basal and squamous skin indications
  ...
  // brain
  { subtype: "atrt", parents: ["brain-tumours", "childhood-indications"], any: [/rhabdoid|\bATRT\b|AT\/RT/i] },
  ...
  { subtype: "medulloblastoma", parents: ["brain-tumours", "childhood-indications"], any: [/medulloblastoma/i] },
  ...
  { subtype: "paediatric-high-grade-glioma", parents: ["glioblastoma", "brain-tumours", "childhood-indications"], any: [/high[- ]grade glioma|\bpHGG\b|glioblastoma/i], all: [PAED], none: [/DIPG|pontine|diffuse midline/i] },
  { subtype: "paediatric-low-grade-glioma", parents: ["glioblastoma", "brain-tumours", "childhood-indications"], any: [/low[- ]grade glioma|\bpLGG\b|pilocytic|optic pathway/i], all: [PAED] },
  // breast
  ...
  // childhood
  { subtype: "hepatoblastoma", parents: ["childhood-indications"], any: [/hepatoblastoma/i] },
  { subtype: "neuroblastoma", parents: ["childhood-indications"], any: [/neuroblastoma/i] },
  { subtype: "retinoblastoma", parents: ["childhood-indications"], any: [/retinoblastoma/i] },
  { subtype: "wilms-tumor", parents: ["childhood-indications"], any: [/Wilms|nephroblastoma/i] },
  { subtype: "paediatric-germ-cell-tumours", parents: ["childhood-indications"], any: [/germ cell|germinoma/i] },
  { subtype: "pleuropulmonary-blastoma", parents: ["childhood-indications"], any: [/pleuropulmonary|\bPPB\b|DICER1/i] },
  ...
  { subtype: "pancreatoblastoma", parents: ["pancreatic", "childhood-indications"], any: [/pancreatoblastoma/i] },
  // penile cancer
  ...
  { subtype: "rhabdomyosarcoma", parents: ["sarcoma", "childhood-indications"], any: [/rhabdomyosarcoma/i] },
...
export function matchTrial(t: TrialEntity, rule: Rule): string | undefined {
  if (t.indications.includes(rule.subtype) || !rule.parents.some((p) => t.indications.includes(p))) return undefined;
  const text = textOf(t);
...
export function plan(opts: PlanOptions = {}): { matches: Match[]; rejected: Array<Match & { reason: string }>; counts: Map<string, number> } {
  ...
  for (const r of RULES) {
    ...
    if (s?.kind !== "indication") throw new Error(`RULES names ${r.subtype}, which is not a cancer record`);
    for (const p of r.parents) if (g.get(p)?.kind !== "indication") throw new Error(`RULES for ${r.subtype} names parent ${p}, which is not a cancer record`);
  }
...
export function toEdits(matches: Match[]): Edit[] {
  return matches.map((m) => ({ targetId: m.trial.id, targetKind: "trial", targetName: m.trial.name, field: "indications", add: m.subtype, why: `subtype token "${m.token}"` }));
}

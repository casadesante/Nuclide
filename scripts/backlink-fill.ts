...
const FIELD_FOR: Partial<Record<Kind, RelField>> = {
  drug: "drugs", indication: "indications", target: "targets", technology: "technologies", company: "companies", institution: "institutions",
...
const LINK_KINDS: Kind[] = ["drug", "indication", "target", "technology", "company", "institution", "trial", "collection"];
...
const SKIP_TOKENS = new Set<string>([
  "indication", "indications", "tumour", "tumor", "solid tumours", "solid tumors", "advanced solid tumours", "metastatic cancer", "metastatic", "leukaemia", "leukemia", "lymphoma", "sarcoma", "carcinoma", "melanoma",
...
function tokensFor(e: Entity): string[] {
  ...
  if (e.kind === "trial") { for (const a of e.aka) add(stripParens(a)); }
    ...
    for (const r of [e.name, ...e.aka]) {
      ...
      if (m && ["target", "technology", "term", "indication"].includes(e.kind) && /[A-Z].*[A-Z0-9]/.test(m[1])) add(m[1]);
    }
...
export function plan(kinds: Set<Kind>): { proposals: Proposal[]; weak: Entity[]; noText: Entity[] } {
  ...
  for (const w of weak) {
    ...
    for (const t of toks) {
      ...
      // Registry trials already carry indications matched from condition terms; do not re-derive them from the title, and
      // registry boilerplate names glossary terms and other trials constantly without meaning them.
      if (w.kind === "trial" && (t.kind === "indication" || t.kind === "trial")) continue;
...
/** Relation arrays whose existing entries may be mirrored back onto the weak record; indications, terms and fronts are left alone so those pages do not fill by reverse edge. */
...
export function reverseEdits(proposals: Proposal[], weak: Entity[], g = graph()): Edit[] {
  ...
  const push = (w: Entity, b: Entity, why: string, always: boolean): void => {
    ...
    if (!always) {
      ...
      if (b.kind === "indication" || b.kind === "term") return;
    }
  ...
  for (const p of proposals) {
    const w = p.from;
    if (w.kind === "trial" && p.add.kind === "indication" && !cancerEligible(w, false)) continue;
  ...
  for (const w of weak) {
    ...
    if (w.kind === "trial" && cancerEligible(w, false)) for (const id of w.indications) push(w, g.must(id), `mirror of ${w.id}.indications (phase ${w.phase}${w.result ? ", result" : ""})`, true);
  }
  return out;
}

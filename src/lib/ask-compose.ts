/**
 * Ask Nuclide, step two: build the answer from the structured fields of the records the question named.
 *
...
export type AskEntity = EntityLike & {
  ...
  symbol?: string; targetClass?: string; prevalence?: Array<{ indicationId: string; pct: number | string; measure?: string }>;
  ...
  targets?: string[]; drugs?: string[]; indications?: string[]; trials?: string[]; companies?: string[]; technologies?: string[]; terms?: string[]; institutions?: string[]; sections?: string[];
};
...
const GRADE_VERDICT: Record<EvidenceGrade, (name: string) => string> = {
  ...
  insufficient: (n) => `For ${n} the evidence is insufficient: laboratory, animal or uncontrolled human data only, or trials too small and inconsistent to draw a conclusion. Nuclide does not present it as a treatment option; it is not a reason to use it outside a trial, and it must never replace standard treatment.`,
  "no-benefit": (n) => `${n} was tested and showed no benefit: adequately sized randomised trials found no effect on the outcome it was claimed to change. Nuclide does not present it as an option.`,
  harm: (n) => `For ${n} there is evidence of harm or interaction: direct toxicity, a clinically important interaction with cancer treatment, or use in place of standard treatment that is associated with worse outcomes. Nuclide does not present it as an option; tell your treatment team before taking anything like it.`,
};
...
function regionalSentences(name: string, id: string, region: Region | undefined): string[] {
  ...
  if (region && !row[region]) out.push(`Nuclide has no regulatory record for ${name} in ${regionName(region)}; absence means unknown, not "not approved".`);
  return out;
}
...
function tDefine(c: Ctx, r: AskEntityRecord): boolean {
  ...
  switch (e.kind) {
    case "indication": if (e.stateOfArt?.[0]) c.b.add(s, e.stateOfArt[0], "state of the art"); break;
    ...
    case "pathway": {
      ...
      if (kegg) c.b.add(s, `This is KEGG map ${kegg}${e.indications?.length ? `, the pathway map for ${list(e.indications.slice(0, 3).map((id) => c.lookup(id)?.name ?? id))}` : ""}.`, "kegg");
    ...
    case "journal": {
      ...
      if (e.impactFactor) c.b.add(s, `Its ${e.impactFactor.year} impact factor was ${e.impactFactor.value}${e.impactFactor.source ? ` (${e.impactFactor.source})` : ""}; Nuclide lists no other metric.`, "impact factor");
      const papers = (r.neighbours.paper ?? []).slice(0, 4).map((p) => p.name);
      if (papers.length) c.b.add(s, `Key papers on Nuclide it carried include ${list(papers)}.`, "linked papers");
    ...
    case "term": {
      ...
      const indications = (r.neighbours.indication ?? []).slice(0, 4).map((d) => shortName(d));
      if (drugs.length || indications.length) c.b.add(s, `Why it matters: ${[drugs.length ? `it decides or shapes the use of ${list(drugs)}` : "", indications.length ? `it comes up in ${list(indications)}` : ""].filter(Boolean).join(", and ")}.`, "linked records");
      break;
    }
  ...
  if (e.kind === "indication" || e.kind === "target") {
    const drugs = keyDrugs(r, c.lookup).slice(0, 5).map((d) => d.name);
    if (drugs.length) c.b.add(s, `${e.kind === "indication" ? "Treatments in the standard of care for" : "Approved products against"} ${short(r)} on Nuclide include ${list(drugs)}.`, "linked products");
    if (e.kind === "indication" && e.burden && /\bhow (?:common|many)\b|\bincidence\b|\bprevalence\b|\bburden\b|\bcases\b|\bmarket\b/i.test(c.question)) c.b.add(s, e.burden, "burden");
  }
...
function tTreatments(c: Ctx, r: AskEntityRecord): boolean {
  ...
  if (e.kind === "drug") {
    ...
    else c.b.add(s, `${short(r)} is not yet approved anywhere Nuclide tracks${e.status ? `; its status is ${e.status.replace("-", " ")}` : ""}.`, "status");
    const indications = (r.neighbours.indication ?? []).slice(0, 5).map((x) => shortName(x));
    if (indications.length) c.b.add(s, `Cancers linked to ${short(r)} on Nuclide: ${list(indications)}.`, "linked indications");
    return true;
  }
  if (e.kind === "target" || e.kind === "technology" || e.kind === "term" || e.kind === "pathway") {
    ...
    if (!approved.length && !late.length) { const other = (r.neighbours.drug ?? []).slice(0, 6).map((d) => withStatus(d, c.lookup)); if (other.length) c.b.add(s, `Products on Nuclide linked to ${short(r)}: ${list(other)}.`, "linked products"); }
...
function tBiomarkers(c: Ctx, r: AskEntityRecord): boolean {
  const s = toSource(r), e = r.entity;
  if (e.kind !== "indication") return false;
  c.b.add(s, e.tldr, "TL;DR");
...
function tApproval(c: Ctx, r: AskEntityRecord): boolean {
  ...
  if (e.kind === "drug") {
    ...
    if (ap.length) c.b.add(s, `Approvals recorded on Nuclide: ${ap.slice(-4).map((a) => `${a.region} ${a.year}, ${a.indication}`).join("; ")}.`, "approvals");
    else if (!regionalApprovals[e.id]) c.b.add(s, `${short(r)} is not approved anywhere Nuclide tracks${e.status ? `; its status is ${e.status.replace("-", " ")}` : ""}.`, "status");
  ...
  if (e.kind === "technology" || e.kind === "target" || e.kind === "term") {
    c.b.add(s, e.tldr, "TL;DR");
    const { approved, late } = drugsOf(r, c.lookup);
    if (approved.length) c.b.add(s, `Approved products ${e.kind === "target" ? "against" : "using"} ${short(r)} on Nuclide: ${list(approved.slice(0, 8).map((d) => d.name))}.`, "linked products");
    else c.b.add(s, `No product ${e.kind === "target" ? "against" : "using"} ${short(r)} is approved yet on Nuclide's record.`, "linked products");
    if (late.length) c.b.add(s, `In phase 3: ${list(late.slice(0, 6).map((d) => d.name))}.`, "linked products");
    if (e.kind === "technology" && e.since) c.b.add(s, `First used in people or approved: ${e.since}.`, "since");
    return true;
  }
  if (e.kind === "indication") return tTreatments(c, r);
  return false;
}
...
function tSideEffects(c: Ctx, r: AskEntityRecord): boolean {
  ...
  if (e.kind === "drug") {
    ...
    if (tox.length) {
      ...
      if (!hits.length) c.b.add(s, `Nuclide has no side-effect table for ${short(r)} yet; the label and the pivotal trial publication are the sources to check.`, "toxicity");
    }
    if (e.dosing?.monitoring) c.b.add(s, `Monitoring: ${e.dosing.monitoring}.`, "dosing");
...
function tTrials(c: Ctx, r: AskEntityRecord): boolean {
  ...
  c.b.add(s, e.tldr, "TL;DR");
  if (trials.length) {
    c.b.add(s, `Trials on Nuclide linked to ${short(r)}: ${list(trials.map((t) => withStatus(t, c.lookup)))}.`, "linked trials");
...
function compareFields(r: AskEntityRecord, lookup: ComposeInput["lookup"]): Array<[string, string]> {
  ...
  switch (e.kind) {
    ...
    case "indication": return [["Burden", e.burden ?? ""], ["Biomarkers", (e.biomarkers ?? []).slice(0, 4).join("; ")], ["Standard of care", (e.standardOfCare ?? []).slice(0, 2).map((x) => `${x.setting}: ${x.approach}`).join(" ")]];
...
function tWho(c: Ctx, r: AskEntityRecord): boolean {
  ...
  switch (e.kind) {
    ...
    case "drug": {
      ...
      if (inst.length) c.b.add(s, `Institutions on Nuclide linked to ${short(r)}: ${list(inst)}.`, "linked institutions");
      for (const x of first(e.summary, 1)) c.b.add(s, x, "summary");
    ...
    case "technology": case "target": case "trial": case "indication": {
      c.b.add(s, e.tldr, "TL;DR");
      ...
      if (cos.length) c.b.add(s, `${viaProducts.size ? "Also linked" : "Companies on Nuclide linked"} to ${short(r)}: ${list(cos)}.`, "linked companies");
      if (inst.length) c.b.add(s, `Institutions: ${list(inst)}.`, "linked institutions");
...
function tEvidence(c: Ctx, r: AskEntityRecord): boolean {
  ...
  c.b.add(s, `Evidence grade on Nuclide: ${GRADE_META[grade].label.toLowerCase()}. ${GRADE_VERDICT[grade](short(r))}`, "evidence grade");
  c.b.add(s, e.tldr, "TL;DR");
  for (const x of first(e.summary, 2)) c.b.add(s, x, "summary");
...
function tRegionalApprovals(c: Ctx, r: AskEntityRecord): boolean {
  ...
  if (all.length) {
    const what = e.kind === "indication" ? `Products for ${short(r)}` : e.kind === "target" ? `Products against ${short(r)}` : `${short(r)} products`;
    c.b.add(s, `${what} approved in ${label} by the ${reg} on Nuclide's record: ${list(all.slice(0, 20).map((d) => describe(d.id, d.name)))}${all.length > 20 ? `, and ${all.length - 20} more` : ""}.`, "regional approvals");
...
function tCompanies(c: Ctx, r: AskEntityRecord): boolean {
  ...
  if (e.kind === "company") {
    ...
    if (batch) {
      ...
      if (chosen.length) {
        c.b.add(s, `Y Combinator companies from the ${batchLabel(batch)} batch working on cancer, on Nuclide: ${list(chosen.map((x) => x.entity.name))}.`, "portfolio");
        for (const x of chosen.slice(0, 4)) c.b.add(toSource(x), x.entity.tldr, "TL;DR");
      } else {
        c.b.add(s, `Nuclide lists no cancer company from the ${batchLabel(batch)} batch; the Y Combinator page lists every batch it covers.`, "portfolio");
      }
      ...
      c.b.add(s, `${portfolio.length} companies on Nuclide name ${short(r)} as an investor${sorted.some((n) => batchOf(n.id)) ? ", most recent batch first" : ""}: ${list(shown)}${portfolio.length > 30 ? `, and ${portfolio.length - 30} more on its page` : ""}.`, "portfolio");
...
function tInvestors(c: Ctx, r: AskEntityRecord): boolean {
  ...
  if (e.kind === "company") {
    ...
    c.b.add(s, companyLine(e), "profile");
    c.b.add(s, e.tldr, "TL;DR");
    if (inv.length) c.b.add(s, `Investors in ${short(r)} on Nuclide's record: ${list(inv)}.`, "investors");
  ...
  if (backed.size) {
    ...
    c.b.add(s, `Investors backing ${short(r)} companies on Nuclide, most companies first: ${list(ranked.slice(0, 10).map(([n, xs]) => `${n} (${list(xs.slice(0, 3))})`))}.`, "investors");
  } else {
    c.b.add(s, `Nuclide records no investor for the ${short(r)} companies it lists; backers appear only when a source states a round.`, "investors");
  }
...
function tRoadmap(c: Ctx, r: AskEntityRecord): boolean {
  ...
  const fit = (x: AskEntityRecord) => { const re = x.entity; const secs = re.sections ?? []; return (secs.includes(e.id) ? 3 : 0) + ([...(re.technologies ?? []), ...(re.indications ?? []), ...(re.targets ?? []), ...(re.drugs ?? [])].includes(e.id) ? 2 : 0) - (secs.length > 3 ? 2 : 0); };
...
function tJournals(c: Ctx, r: AskEntityRecord): boolean {
  ...
  if (e.kind === "journal") {
    ...
    c.b.add(s, `Journals on Nuclide whose name or scope covers "${c.topic.words}": ${list(entries.slice(0, 12).map(one))}${entries.length > 12 ? `, and ${entries.length - 12} more` : ""}.`, "matched journals");
  ...
  if (linked.length) c.b.add(s, `Journal record${linked.length > 1 ? "s" : ""} on Nuclide: ${list(linked.slice(0, 4).map((x) => x.name))}.`, "linked journals");
  for (const x of others.slice(0, 2)) c.b.add(toSource(x), x.entity.tldr, "TL;DR");
  if (!e.journal && !linked.length && !others.length) c.b.add(s, `Nuclide has no journal linked to ${short(r)} yet.`, "linked journals");
  c.readMore.push({ label: "Journals", href: "/journals/" });
  return true;
}
...
export function followUpsFor(e: { kind: Kind; name: string }, intent: Intent): string[] {
  ...
  const by: Partial<Record<Kind, Array<[Intent, string]>>> = {
    indication: [["treatments", `How is ${n} treated?`], ["trials", `What trials are open for ${n}?`], ["biomarkers", `Which biomarkers matter in ${n}?`], ["define", `What is ${n}?`]],
...
const DETAIL_KINDS = new Set<Kind>(["indication", "drug", "target", "technology", "trial", "term", "paper", "pathway", "company", "institution", "person", "journal", "roadmap"]);
...
export function composeTemplated(input: ComposeInput): AskAnswer {
  ...
  if (applied) {
    ...
    if (!answer.sentences.length && !recs.length) answer.note = "Nuclide has no record close to this question. Try naming a cancer, product, target, trial or term.";
  }

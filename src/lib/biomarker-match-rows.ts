...
export function matchRows(): MatchRow[] {
  ...
  for (const c of g.kind("indication")) for (const list of g.forCancer(c.id).values()) for (const e of list) (relevance.get(e.id) ?? relevance.set(e.id, new Set()).get(e.id)!).add(c.id);
  ...
  return g.entities.filter((e) => KINDS.includes(e.kind)).map((e) => {
    ...
    return {
      ...
      indications: [...(relevance.get(e.id) ?? [])],

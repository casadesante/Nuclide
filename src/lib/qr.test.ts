...
describe("qr", () => {
  ...
  it("picks the smallest version that fits and sizes the matrix accordingly", () => {
    const small = qrEncode("nuclide.cc", "M");
    ...
    const url = qrEncode("https://nuclide.cc/drugs/trastuzumab-deruxtecan/", "M");
  ...
  it("draws the three finder patterns, the timing patterns and the dark module", () => {
    const { modules: m, size } = qrEncode("https://nuclide.cc/", "M");
  ...
  it("is deterministic and every module is a boolean", () => {
    const a = qrEncode("https://nuclide.cc/indications/tnbc/");
    const b = qrEncode("https://nuclide.cc/indications/tnbc/");
    expect(a).toEqual(b);

...
export function trialLeadership(): LeadershipRow[] {
  ...
  const rows = g.kind("institution").map((inst) => {
    ...
    const indications = n.get("indication") ?? [];
    return { institution: inst, trials, drugs, technologies, targets, indications, total: trials.length * 3 + drugs.length * 2 + technologies.length + targets.length, rank: 0 };
  });

/**
 * Nuclide graph query language: a one-line text form of the /query/ builder.
 *
 *   indication:tnbc -> drugs(status:approved) -> targets
 *   targets has drugs(status:approved) no technologies(tag:pet-target)
 *   indications no drugs(status:phase-3)
...
export function parse(src: string): Query {
  ...
  const ref = (): Ref => {
    const w = next();
    if (!w || w.t !== "word") throw new GqlError("Expected a kind such as drugs, targets or indication:tnbc", w?.pos ?? src.length);

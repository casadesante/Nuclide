 ...
 *
 * Nuclide dates come at four resolutions: a day (YYYY-MM-DD), a month (YYYY-MM), a quarter (YYYY-Qn) or a
...
export type IcsEvent = {
  uid: string;
  /** Any Nuclide date string (see above). */
  date: string;
  summary: string;
  description?: string;
  url?: string;
...
export function toIcs(events: IcsEvent[], opts: { name: string; description?: string; stamp: Date; domain?: string }): string {
  const domain = opts.domain ?? "nuclide.cc";
  const stamp = opts.stamp.toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
  const lines: string[] = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Nuclide//Catalyst calendar//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",

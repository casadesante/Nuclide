...
export type SearchDoc = { id: string; kind: Kind | "page"; name: string; aka: string; tldr: string; tags: string; route: string; status?: string; /** Space-separated ids of the indications the record links to, for the "for my cancer" filter. */ indications?: string };
...
export const SITE_PAGES: ReadonlyArray<{ href: string; label: string; blurb: string }> = [
  { href: "/about/", label: "About and methodology", blurb: "What Nuclide is, how it is built, its rules for facts, licences and how to contribute." },
  { href: "/terms-of-use/", label: "Terms of use", blurb: "The rules for using Nuclide: not medical advice, accuracy not guaranteed, accounts, licences, liability and contact." },
  { href: "/privacy/", label: "Privacy policy", blurb: "What Nuclide collects and where it lives: hosting, analytics, accounts, browser storage, your rights and how to contact us." },
];
...
export function searchDocs(): SearchDoc[] {
  return graph().entities.map((e) => ({
    ...
    route: routeFor(e),
    status: e.status,
    indications: e.indications.length ? e.indications.join(" ") : undefined,
  }));
}

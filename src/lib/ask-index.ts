/**
 * The compact entity name index Ask Nuclide resolves questions against, and the curated question pairs.
 *
...
/** What to call a record in a generated sentence or follow-up: the abbreviation for indications when there is one, else the base name. */
export function shortName(e: { kind: Kind; name: string }): string {
  const abbr = abbreviation(e.name);
  if (abbr && (e.kind === "indication" || e.kind === "term" || e.kind === "technology") && /^[A-Z0-9\-/]{2,8}$/.test(abbr)) return abbr;

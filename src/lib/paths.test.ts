...
const toy: PathData = {
  nodes: [
    ...
    { id: "tnbc", kind: "indication", name: "TNBC", route: "/indications/tnbc/" },
    { id: "hub", kind: "section", name: "ADCs front", route: "/fronts/hub/" },
  ],
  vias: ["pathway-node", "targets", "indications", "sections"],
  edges: [
...
describe("findPaths", () => {
  ...
  it("labels hops in both directions", () => {
    ...
    expect(hopLabel("standardOfCare", false, "drug", "indication")).toBe("standard of care for");
  });
});

describe("corpus paths", () => {
  it("connects a pathway to a radioligand therapy within a few hops", () => {
    const data = pathData();
    const idx = new Map(data.nodes.map((n, i) => [n.id, i]));
    const from = idx.get("prostate-cancer-signalling") ?? 0;
    const to = idx.get("pluvicto")!;

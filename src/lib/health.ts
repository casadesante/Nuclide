...
const KIND_PRIORITY: Kind[] = ["indication", "drug", "target", "trial", "technology", "pathway", "company", "institution", "person", "paper", "journal", "collection", "bottleneck", "opportunity", "roadmap", "pairing", "term", "idea", "section"];
...
export const METRIC_DEFS: MetricDef[] = [
  ...
  },
  {
    ...
    action: "Add ids to the typed relation arrays (`indications`, `targets`, `drugs`, `trials`, `terms`...) on this record or on the records that should point to it.",
  ...
  {
    id: "indication-depth", label: "Indications with a deep dive", kind: "indication",
    ...
    action: "Add `standardOfCare` rows by setting with refs, `history` events with refs, and `pipeline` ids; see indications.ts for the TNBC model.",
    check: (g) => fails(g.kind("indication"), (c) => {
  ...
  },
  {
    ...
    action: "Add `prevalence` rows ({ indicationId, pct, measure, source }) to the target record.",
  ...
  },
  {
    ...
    check: (g, ctx) => fails(g.entities.filter((e) => ["drug", "target", "indication", "technology"].includes(e.kind)), (e) => (ctx.papers.has(e.id) ? null : "no papers snapshot")),
  },
  {
...
function issueUrl(m: Omit<HealthMetric, "issueUrl">): string {
  const body = [
    ...
    "", `Gauge: https://nuclide.cc/roadmap/#metric-${m.id}`, `Check: ${m.code}`,
  ].join("\n");

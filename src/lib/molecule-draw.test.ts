...
describe("small molecule scene (dotatate, PubChem 3D conformer)", () => {
  const mol = load("pubchem-11170867.json");
  ...
  it("keeps every atom and bond, lists CPK elements, and fits a sensible radius", () => {
    ...
    expect(scene.elements).toEqual(expect.arrayContaining(["C", "N", "O", "S", "H"]));
...
describe("protein scene (5O5T, PSMA with the PSMA-1007 ligand)", () => {
  const mol = load("pdb-5O5T.json");
  ...
  it("uses the recorded secondary structure, keeps only the ligand as atoms, and centres on it", () => {
    ...
    expect(scene.rib.count).toBeGreaterThan(scene.residues * 3); // the ribbon is interpolated at several points per residue
...
describe("multi-chain complex (7T10, SSTR2 with Gi3)", () => {
  it("gives each chain its own colour index and draws without error", () => {
    const scene = buildScene(load("pdb-7T10.json"), true);
    expect(scene.chains.length).toBe(6); // receptor, the three G-protein subunits, the peptide and the fiducial nanobody
    expect(scene.hasLigand).toBe(false);
    expect(new Set(Array.from(scene.rib.chain)).size).toBe(6);
    const r = createRenderer(scene, fakeSprite);
    const { ctx, ops } = recorder();

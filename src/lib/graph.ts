...
class Graph {
  ...
  /** Everything relevant to a indication: direct links plus things linked to its drugs/targets. */
  forCancer(indicationId: string): Map<Kind, Entity[]> {
    const direct = this.neighbours(indicationId);
    const out = new Map<Kind, Set<Entity>>();
    const add = (n: Entity) => {
      if (n.id === indicationId) return;

...
/** Muted, mid-saturation hue per kind. Pink is kept for indications (the site accent); everything else sits a step quieter. */
...
export const ORDER: Kind[] = ["indication", "section", "technology", "target", "drug", "trial", "pairing", "pathway", "company", "institution", "person", "bottleneck", "opportunity", "paper", "journal", "idea", "roadmap", "term", "collection"];
...
/** Fronts on the inner ring in corpus order, indications on the outer ring alphabetically, both starting at the top. */
export function layoutOverview(data: GraphData): Scene {
  const placed: Placed[] = [];
  const fronts = data.nodes.map((n, i) => ({ n, i })).filter(({ n }) => n.kind === "section");
  const indications = data.nodes.map((n, i) => ({ n, i })).filter(({ n }) => n.kind === "indication").sort((p, q) => p.n.name.localeCompare(q.n.name));
  ...
  indications.forEach(({ n, i }, k) => {
    const a = (k / indications.length) * Math.PI * 2 - Math.PI / 2;

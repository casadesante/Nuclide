/**
 * Ask Nuclide: grounded, cited answers assembled from record text only.
 *
...
const GENERIC = new Set(["indication", "patient", "treatment", "drug", "trial", "study", "use", "used", "also", "one", "two", "first", "new", "year", "high", "low", "cell", "disease"]);
...
export function composeAnswer(question: string, records: AskRecord[], opts: ComposeOptions = {}): Answer {
  ...
  if (!records.length) return { sentences: [], sources, confidence: "low", note: "Nuclide has no record close to this question." };
  ...
  const note = confidence === "low" ? "No record answers this directly. The sentences below are the closest Nuclide has; check the linked pages." : undefined;
...
export function answerText(a: Answer): string {
  ...
  const refs = a.sources.map((s, i) => `[${i + 1}] ${s.name} (https://nuclide.cc${s.route})`).join("\n");
  return `${body}\n\nSources\n${refs}`;
}

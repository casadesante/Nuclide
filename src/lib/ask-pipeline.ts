/**
 * Ask Nuclide, end to end: analyse the question (ask-intent.ts), decide which records to read, fetch them,
...
const JOURNAL_STOP = new Set(["which", "what", "journal", "journals", "publish", "publishes", "published", "publishing", "cover", "covers", "covering", "carry", "research", "papers", "paper", "about", "for", "on", "in", "the", "a", "an", "of", "and", "to", "do", "does", "are", "is", "where", "should", "i", "read", "oncology", "indication", "best", "top", "good", "clinical", "field", "work", "studies", "trials"]);
...
export async function answerQuestion(question: string, deps: AskDeps): Promise<AskResult> {
  ...
  if (a.intent === "journals" && (listing || !primaryEntry || primaryEntry.kind !== "journal")) {
    ...
    const anchored = primaryEntry && primaryStrong && !["term", "technology", "indication", "section", "journal"].includes(primaryEntry.kind);

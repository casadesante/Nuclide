...
const Base = z.object({
  ...
  indications: z.array(id).default([]),
...
export const IndicationSchema = Base.extend({
  kind: z.literal("indication"),
...
export const TargetSchema = Base.extend({
  ...
  prevalence: z.array(z.object({ indicationId: id, pct: z.union([z.number(), z.string()]), measure: z.string().optional(), source: url.optional(), note: z.string().optional() })).default([]),
});


/** A medical radionuclide: what it emits, how long it lasts, how it is made, and how tight supply is. */
export const IsotopeSchema = Base.extend({
  kind: z.literal("isotope"),
  /** Symbol as the field writes it, e.g. "177Lu", "225Ac", "99mTc". */
  symbol: z.string(),
  element: z.string(),
  massNumber: z.number().int().optional(),
  /** Physical half-life in words, e.g. "6.65 days". */
  halfLife: z.string(),
  /** What it emits, one per item, with energies where a source states them. */
  emissions: z.array(z.string()).default([]),
  /** Primary particle class, for filtering. */
  emissionClass: z.enum(["alpha", "beta-minus", "positron", "gamma", "auger", "mixed"]),
  /** Therapy, imaging, or both halves of a theranostic pair. */
  use: z.enum(["therapy", "imaging", "both"]),
  /** Mean or maximum particle range in soft tissue, in words. */
  tissueRange: z.string().optional(),
  /** Reactor, cyclotron, generator, accelerator, or decay of a parent, in words. */
  production: z.string(),
  /** How tight supply is today. */
  supply: z.enum(["adequate", "tight", "constrained", "emerging", "research-only"]).optional(),
  supplyNote: z.string().optional(),
  /** Chelators or chemistry it is usually paired with. */
  chelators: z.array(z.string()).default([]),
  /** Isotope ids it forms a theranostic pair with (68Ga with 177Lu, 64Cu with 67Cu). */
  pairedWith: z.array(id).default([]),
  /** First medical use, if meaningful. */
  since: z.union([z.number().int(), z.string()]).optional(),
});
...
export const CompanySchema = Base.extend({
  ...
  /** Company id of the acquirer, when `stage` is "acquired" and the acquirer is in Nuclide. */
...
export type Isotope = z.infer<typeof IsotopeSchema>;
...
export type IndicationInput = z.input<typeof IndicationSchema>;
...
export type IsotopeInput = z.input<typeof IsotopeSchema>;

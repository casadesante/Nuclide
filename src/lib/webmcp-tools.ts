...
export function createWebMCPTools(): WebMCPTool[] {
  return [
    tool("onco_search", "Search Nuclide's public oncology records by name, alias, or keywords (lexical search). Returns record IDs, summaries and page URLs; optionally filter by kind. Not personalised medical advice.", searchInput, async ({ query, kind, limit }) => {
    ...
    tool("onco_get_entity", "Read a public Nuclide record using an exact ID from onco_search. Returns the full dated record, primary-source links and connected records. Verify facts at source; not medical advice.", entityInput, async ({ id }) => {

...
const disclaimer = "Nuclide is an orientation tool, not medical advice. Records may be incomplete or out of date; verify claims at their linked primary sources.";
  ...
  return {
    ...
    async execute(input) {
      ...
      try {
        ...
        return { isError: true, error: "Nuclide data could not be loaded. Check the connection and try again, or use the site search.", attribution: EXPORT_LICENCE, disclaimer };
      }
    },
  };
}
...
export function createWebMCPTools(): WebMCPTool[] {
  return [
    tool("nuclide_search", "Search Nuclide's public radiopharmaceutical records by name, alias, or keywords (lexical search). Returns record IDs, summaries and page URLs; optionally filter by kind. Not personalised medical advice.", searchInput, async ({ query, kind, limit }) => {
    ...
    tool("nuclide_get_entity", "Read a public Nuclide record using an exact ID from nuclide_search. Returns the full dated record, primary-source links and connected records. Verify facts at source; not medical advice.", entityInput, async ({ id }) => {

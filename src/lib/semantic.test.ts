...
describe("tokenize", () => {
  it("normalises spelling, splits compounds and stems plurals", () => {
    ...
    expect(tokenize("HER2-low breast indications")).toEqual(["her2-low", "her2", "low", "breast", "indication"]);
...
describe("corpus index", () => {
  it("builds for the whole corpus and finds Pluvicto for the lutetium-PSMA paraphrase", { timeout: 120_000 }, () => {
    const index = buildSemanticIndex(semanticDocs());
    expect(index.ids.length).toBeGreaterThan(700);
    const top = semanticSearch(index, "lutetium PSMA radioligand therapy", 10).map((h) => h.id);
    expect(top).toContain("pluvicto");
  });
});

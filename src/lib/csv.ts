...
export const EXPORT_LICENCE = "Data from Nuclide (https://nuclide.cc), CC BY-NC 4.0. Free for individual and educational use with attribution: name Nuclide and link to https://nuclide.cc wherever the data or derived text appears. Commercial use must contact Nuclide to pay for the data (https://nuclide.cc/about/#licence).";
...
/** `nuclide-<slug>-YYYY-MM-DD.<ext>`; the slug keeps letters, digits and hyphens only. */
export function exportFilename(name: string, ext: "csv" | "json", date = new Date()): string {
  ...
  return `nuclide-${slug}-${date.toISOString().slice(0, 10)}.${ext}`;
}
...
export function toJsonExport(rows: unknown[], meta: { name: string; source?: string; date?: Date }): string {
  return JSON.stringify({ name: meta.name, source: meta.source ?? "https://nuclide.cc", exported: (meta.date ?? new Date()).toISOString(), licence: "CC BY-NC 4.0", attribution: EXPORT_LICENCE, count: rows.length, rows }, null, 0);
}

...
describe("corpus", () => {
  ...
  it("the prostate cancer spike is fully built", () => {
    const g = graph();
    const t = g.must("prostate");
    expect(t.kind).toBe("indication");
    if (t.kind !== "indication") return;
    ...
    const rel = g.forCancer("prostate");
  ...
  it("backlinks are symmetric with outgoing links", () => {
    const g = graph();
    const psma = g.incoming("psma");
    const drugs = psma.get("drug") ?? [];
    expect(drugs.some((d) => d.id === "pluvicto")).toBe(true);
  });

...
describe("graph explorer layout", () => {
  ...
  it("puts fronts on the inner ring and indications on the outer ring, alphabetically", () => {
    const s = layoutOverview(data);
    const fronts = s.placed.filter((p) => p.n.kind === "section");
    const indications = s.placed.filter((p) => p.n.kind === "indication");
    expect(fronts.length).toBeGreaterThan(10);
    expect(indications.length).toBeGreaterThan(40);
    for (const p of fronts) expect(Math.hypot(p.x, p.y)).toBeCloseTo(100, 5);
    for (const p of indications) expect(Math.hypot(p.x, p.y)).toBeCloseTo(224, 5);
    const names = indications.map((p) => p.n.name);
  ...
  it("staggers and shrinks a crowded ring instead of overlapping labels", () => {
    ...
    // Force a small number of sub-rings so the most-connected node in this corpus is still crowded
    // enough to require staggering and shrinking (its natural degree is lower than OnCo's TNBC spike).
    const s = layoutFocus(data, adj, i, new Set(), all, 2);

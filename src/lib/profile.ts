...
/**
 * A browser-only profile that personalises Nuclide. Stored in localStorage under one key.
...
export function saveProfile(p: Profile) {
  ...
  window.dispatchEvent(new CustomEvent("nuclide:profile", { detail: p }));
}

export function clearProfile() {
  if (typeof window === "undefined") return;
  window.localStorage.removeItem(KEY);
  window.dispatchEvent(new CustomEvent("nuclide:profile", { detail: EMPTY }));
}
...
export function useProfile(): [Profile, (patch: Partial<Profile>) => void, boolean, () => void] {
  ...
  useEffect(() => {
    ...
    window.addEventListener("nuclide:profile", onChange);
    window.addEventListener("storage", onStorage);
    return () => { cancelAnimationFrame(id); window.removeEventListener("nuclide:profile", onChange); window.removeEventListener("storage", onStorage); };
  }, []);
...
export function isProfileEmpty(p: Profile): boolean {
  return !p.indicationId && p.stage === "unknown" && p.biomarkers.length === 0 && p.priorLines.length === 0 && !p.country && !p.postcode && !p.setting && p.hadTreatments.length === 0 && p.wantsTrials && !p.diagnosedRecently;
}

...
import type { Indication, Drug, Trial } from "./schema";
import { routeFor } from "./kinds";
import type { Graph } from "./graph";

/**
 * "What changed" for one indication: dated events read straight off the records that link to it. Nothing here is
...
/** Words that identify a cancer in free text: its name without brackets, its aliases and its id with hyphens as spaces. Subtypes are left out: many are biomarker names shared across indications. */
export function cancerNeedles(c: Pick<Indication, "id" | "name" | "aka">): string[] {
 ...
 * cancer alone is taken to be about it; a drug used in several indications only contributes lines that name this one.
 */
...
export function changesForCancer(g: Graph, c: Indication): ChangeItem[] {
  ...
  for (const d of (near.get("drug") ?? []) as Drug[]) {
    // Trust a drug only when its own indications field names this cancer alone; anything reached indirectly must name the cancer in the text.
    const n = d.indications.includes(c.id) ? d.indications.length : Number.POSITIVE_INFINITY;
  ...
  for (const h of c.history) {
    ...
    items.push({ date: String(y), kind: "history", title: h.title, detail: h.note, href: ref ? routeFor(ref) : `${cancerHref}#history`, ref: ref ? { id: ref.id, name: ref.name, kind: ref.kind } : { id: c.id, name: c.name, kind: "indication" }, field: "history" });
  }
  ...
  for (const s of c.standardOfCare) {
    ...
    items.push({ date: String(y), kind: "guideline", title: `Guideline ${s.guideline.version}: ${s.setting}`, detail: bits ? `${s.approach} (${bits})` : s.approach, href: s.guideline.url ?? `${cancerHref}#care`, ref: { id: c.id, name: c.name, kind: "indication" }, field: "standardOfCare.guideline.version" });
  }

  if (c.provenance) items.push({ date: c.provenance.editedOn, kind: "record", title: `Record edited by ${c.provenance.editedBy}`, detail: c.provenance.note, href: cancerHref, ref: { id: c.id, name: c.name, kind: "indication" }, field: "provenance.editedOn" });
  items.push({ date: c.asOf, kind: "record", title: "Facts on this page last checked", href: cancerHref, ref: { id: c.id, name: c.name, kind: "indication" }, field: "asOf" });

/**
 * Concept search: a compact, deterministic vector index over every Nuclide record, computed at build time
 ...
 * scored by cosine similarity. Because a product's document includes the names of the indications, targets,
...
const KIND_WORDS: Record<string, string> = {
  ...
  indication: "indication", tumor: "indication", carcinoma: "indication", leukemia: "indication", lymphoma: "indication", sarcoma: "indication", melanoma: "indication", glioma: "indication", myeloma: "indication",
...
export function decodeSemanticIndex(bin: ArrayBuffer, meta: SemanticMeta): SemanticIndex {
  ...
  let p = 0;
  if (v.getUint32(p) !== MAGIC) throw new Error("Not an Nuclide semantic index");
  p += 8;

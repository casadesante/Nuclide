...
const data: GqlData = {
  nodes: [
    { id: "tnbc", kind: "indication", name: "TNBC", route: "/indications/tnbc/", tags: [], tldr: "Triple-negative breast cancer", degree: 3 },
    { id: "nsclc", kind: "indication", name: "NSCLC", route: "/indications/nsclc/", tags: [], tldr: "Lung cancer", degree: 1 },
...
describe("gql parse", () => {
  it("parses hops, ids, filters and limit", () => {
    const q = parse("indication:tnbc -> drugs(status:approved) -> targets | limit:5");
    expect(q.stages.length).toBe(3);
    expect(q.stages[0]).toMatchObject({ kind: "indication", id: "tnbc" });
...
describe("gql run", () => {
  it("walks hops and applies filters", () => {
    const r = query("indication:tnbc -> drugs(status:approved) -> targets", data);
  ...
  it("keeps items by existence of neighbours", () => {
    ...
    expect(query("indications no drugs(status:phase-3)", data).nodes.map((n) => n.id)).toEqual(["nsclc"]);
  });
  ...
  it("describes itself in English", () => {
    expect(describeQuery(parse("indication:tnbc -> drugs(status:approved) -> targets"))).toBe("indication tnbc, then their drugs with status approved, then their targets");
...
describe("gql complete", () => {
  it("suggests kinds at the start and after an arrow", () => {
    ...
    expect(complete("indication:tnbc -> dr", data).options.map((o) => o.value)).toEqual(["drugs"]);
  });
  it("suggests ids after kind: and filter keys inside parentheses", () => {
    expect(complete("indication:t", data).options.map((o) => o.value)).toEqual(["tnbc"]);

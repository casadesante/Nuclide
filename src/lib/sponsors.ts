 ...
 * corpus indications by name and alias, so a study counts under a cancer only when the condition text names it.
...
function cancerMatcher(): (text: string) => string[] {
  const g = graph();
  const STOP = new Set(["indication", "tumour", "tumor", "carcinoma", "neoplasm", "leukaemia", "leukemia", "lymphoma", "sarcoma", "all", "cll", "aml", "mds"]);
  const entries: Array<[string, string]> = [];
  for (const c of g.kind("indication")) {
...
export function sponsorLeaderboard(): { rows: SponsorRow[]; registryStudies: number; registryFetched?: string; unmatchedShare: number } {
  ...
  const rowFor = (name: string): SponsorRow => {
    ...
    if (!r) {
      ...
      r = { key, label: e?.name ?? t.label, id: t.id, route: e ? routeFor(e) : undefined, kind: t.kind, matched: t.matched, corpusTrials: [], registryTotal: 0, registryPhase3: 0, registryPhase2: 0, registryRecruiting: 0, indications: {}, drugs: [] };
      rows.set(key, r);
    }
    return r;
  };
  const bump = (r: SponsorRow, indications: string[]) => { for (const c of indications) r.indications[c] = (r.indications[c] ?? 0) + 1; };

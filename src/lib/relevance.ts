...
export function personalSignals(r: PowerRow, indicationId: string | null, p: Personal): PersonalSignals {
  ...
  if (p.stage && p.stage !== "unknown") {
    const settings = indicationId ? (r.soc[indicationId] ?? []) : Object.values(r.soc).flat();
...
export function scoreRow(r: PowerRow, indicationId: string | null, personal?: PersonalSignals): number {
  let s = r.evidence + 0.5 * Math.min(r.degree, 20);
  if (indicationId) {
    const sig = r.rel[indicationId];

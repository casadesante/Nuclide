...
export function buildBrowser(k: Kind): { rows: BrowserRow[]; facets: FacetDef[]; columns: ColDef[]; hideStatus?: boolean; hideTldr?: boolean; defaultSort?: { key: string; dir: 1 | -1 } } {
  ...
  const borrowed = (e: Entity, withMolecule = false): Partial<BrowserRow> => {
    if (e.indications[0]) return { cancerIcon: e.indications[0] };
  ...
  switch (k) {
    ...
    case "isotope": return {
      hideStatus: true,
      rows: g.kind("isotope").map((i) => ({
        ...base(i), sub: i.symbol,
        facets: { use: [i.use === "both" ? "Therapy and imaging" : cap(i.use)], emission: [cap(i.emissionClass.replace("-", " "))], supply: i.supply ? [cap(i.supply)] : [], tags: i.tags },
        cols: {
          halfLife: i.halfLife,
          emission: fl("emission", cap(i.emissionClass.replace("-", " "))),
          use: fl("use", i.use === "both" ? "Therapy and imaging" : cap(i.use)),
          production: i.production,
          agents: count(inc(i.id, "drug").length, i, "products", "product", "labelled with"),
        },
        sortKeys: { agents: inc(i.id, "drug").length },
      })),
      facets: [{ key: "use", label: "Used for", searchable: false }, { key: "emission", label: "Emission", searchable: false }, { key: "supply", label: "Supply", searchable: false }, { key: "tags", label: "Tag", searchable: false, width: "w-40" }],
      columns: [{ key: "halfLife", label: "Half-life" }, { key: "emission", label: "Emission", hide: "hidden sm:table-cell" }, { key: "use", label: "Used for", hide: "hidden md:table-cell" }, { key: "agents", label: "Products", sortable: true, numeric: true }, { key: "production", label: "How it is made", hide: "hidden xl:table-cell" }],
      defaultSort: { key: "agents", dir: -1 },
    };
    case "section": return {
      hideStatus: true,
    ...
    case "technology": return {
      rows: g.kind("technology").map((t) => ({ ...base(t), schematic: { id: t.id, sections: t.sections }, facets: { front: t.sections.map((id) => g.must(id).name), indications: names(t.indications), targets: names(t.targets), tags: t.tags }, cols: { front: links(t.sections), since: t.since, generation: t.generation, drugs: count(inc(t.id, "drug").length, t, "products", "product", "built on") }, sortKeys: { since: typeof t.since === "number" ? t.since : 0, drugs: inc(t.id, "drug").length } })),
      facets: [{ key: "front", label: "Front", searchable: false, width: "w-52" }, { key: "indications", label: "Indication", width: "w-52" }, { key: "targets", label: "Target" }, { key: "tags", label: "Tag", searchable: false, width: "w-40" }],
    ...
    case "target": return {
      hideStatus: true,
      rows: g.kind("target").map((t) => ({ ...base(t), sub: t.symbol, target: { id: t.id, name: t.name, targetClass: t.targetClass, tldr: t.tldr }, facets: { class: [cap(t.targetClass.replace("-", " "))], indications: names(t.indications), tags: t.tags }, cols: { class: fl("class", cap(t.targetClass.replace("-", " "))), drugs: count(inc(t.id, "drug").length, t, "products", "product", "aimed at"), techs: count(inc(t.id, "technology").length, t, "connected", "technology", "aimed at"), indications: links(t.indications) }, sortKeys: { drugs: inc(t.id, "drug").length, techs: inc(t.id, "technology").length } })),
      facets: [{ key: "class", label: "Class", searchable: false }, { key: "indications", label: "Indication", width: "w-52" }, { key: "tags", label: "Tag", searchable: false, width: "w-40" }],
      columns: [{ key: "class", label: "Class", hide: "hidden sm:table-cell" }, { key: "drugs", label: "Products", sortable: true, numeric: true }, { key: "techs", label: "Technologies", sortable: true, numeric: true, hide: "hidden md:table-cell" }, { key: "indications", label: "Indications", hide: "hidden lg:table-cell" }],
      defaultSort: { key: "drugs", dir: -1 },
    };
    case "drug": {
      ...
      return {
        rows: g.kind("drug").map((d) => {
          ...
          return { ...base(d), molecule: d.id, modality: d.modality, sub: [d.brand, d.code].filter(Boolean).join(" · "), facets: { indications: names(d.indications), modality: [modalityClass(d.modality)], targets: names(d.targets), companies: names(d.companies), front: frontsOf(d), payload: pc ? [pc] : [] }, cols: { modality: d.technologies.length ? [{ label: modalityClass(d.modality), href: routeFor(g.must(d.technologies[0])), tip: g.must(d.technologies[0]).tldr }] : fl("modality", modalityClass(d.modality)), targets: links(d.targets), indications: links(d.indications), companies: links(d.companies), approved: first ? (last && last !== first ? `${first} to ${last}` : String(first)) : undefined }, sortKeys: { approved: last ?? 0 } };
        }),
        facets: [{ key: "indications", label: "Indication", width: "w-56" }, { key: "modality", label: "Modality", searchable: false }, { key: "targets", label: "Target", width: "w-44" }, { key: "companies", label: "Company" }, { key: "front", label: "Front", searchable: false, width: "w-44" }, { key: "payload", label: "ADC payload", searchable: false, width: "w-44" }],
        columns: [{ key: "modality", label: "Modality", sortable: true, hide: "hidden sm:table-cell" }, { key: "targets", label: "Targets", hide: "hidden md:table-cell" }, { key: "indications", label: "Indications", hide: "hidden lg:table-cell" }, { key: "companies", label: "Companies", hide: "hidden lg:table-cell" }, { key: "approved", label: "Approved", sortable: true, numeric: true }],
      };
    }
    case "company": {
      ...
      return {
        ...
        rows: g.kind("company").map((c) => {
          ...
          return {
            ...
            facets: { type: [label[c.companyType] ?? c.companyType], stage: stageLabel ? [stageLabel] : [], country: [c.country], front: c.sections.map((id) => g.must(id).name), indications: names(c.indications), investor: c.investors.map((id) => g.must(id).name) },
        ...
        facets: [{ key: "type", label: "Type", searchable: false, width: "w-52" }, { key: "stage", label: "Stage", searchable: false, width: "w-44", order: STAGE_ORDER.map((s) => STAGE_LABEL[s]) }, { key: "country", label: "Country", searchable: false, width: "w-40" }, { key: "front", label: "Front", searchable: false, width: "w-44" }, { key: "indications", label: "Indication", width: "w-52" }, { key: "investor", label: "Investor", width: "w-52" }],
        columns: [{ key: "type", label: "Type", sortable: true, hide: "hidden sm:table-cell" }, { key: "stage", label: "Stage", sortable: true, hide: "hidden md:table-cell", tip: "Startup, growth stage, public, large private, acquired or wound down. Listed companies default to public; click a chip to filter." }, { key: "hq", label: "HQ", hide: "hidden md:table-cell" }, { key: "country", label: "Country", sortable: true, hide: "hidden lg:table-cell", tip: "Country of the headquarters, as a two-letter code." }, { key: "products", label: "Products", sortable: true, numeric: true, tip: "Products linked to the company; for investors, the number of portfolio companies in Nuclide." }, { key: "techs", label: "Technologies", sortable: true, numeric: true, hide: "hidden lg:table-cell" }],
    ...
    case "institution": {
      ...
      return {
        ...
        rows: ranked.map((r) => { const i = r.institution; return { ...base(i), logo: logoFor(i.id, i.website), avatar: "org", sub: [i.university, `${i.city}, ${i.country}`].filter(Boolean).join(" · "), facets: { type: [cap(i.institutionType.replace("-", " "))], country: [i.country], nci: i.nci ? [cap(i.nci)] : [], indications: names(i.indications) }, cols: { rank: r.rank, type: fl("type", cap(i.institutionType.replace("-", " "))), country: fl("country", i.country), newsweek: i.newsweekOncology2026, nci: fl("nci", i.nci ? cap(i.nci) : undefined), links: count(r.links, i, "connected", "linked object", "at"), score: r.score }, sortKeys: { rank: r.rank, newsweek: i.newsweekOncology2026 ?? 999, links: r.links, score: r.score } }; }),
        facets: [{ key: "type", label: "Type", searchable: false, width: "w-48" }, { key: "country", label: "Country", searchable: false, width: "w-40" }, { key: "nci", label: "NCI", searchable: false, width: "w-40" }, { key: "indications", label: "Indication", width: "w-52" }],
    ...
    case "pathway": return {
      hideStatus: true,
      rows: g.kind("pathway").map((p) => ({ ...base(p), ...borrowed(p), facets: { indications: names(p.indications) }, cols: { nodes: p.nodes.length, targets: links(p.targets), drugs: count(p.drugs.length, p, "connected", "product", "acting on") }, sortKeys: { nodes: p.nodes.length, drugs: p.drugs.length } })),
      facets: [{ key: "indications", label: "Indication", width: "w-52" }],
    ...
    case "trial": return {
      rows: g.kind("trial").map((t) => ({ ...base(t), ...borrowed(t), sub: t.nct, facets: { phase: [phaseLabel(t.phase)], indications: names(t.indications), sponsor: sponsorParts(t.sponsor), drugs: names(t.drugs) }, cols: { phase: fl("phase", phaseLabel(t.phase)), indications: links(t.indications), drugs: links(t.drugs), sponsor: sponsorCell(t.sponsor), year: t.yearReported }, sortKeys: { year: t.yearReported ?? 0 }, tie: t.yearReported ?? 0 })),
      facets: [{ key: "indications", label: "Indication", width: "w-56" }, { key: "phase", label: "Phase", searchable: false, width: "w-40", order: PHASE_ORDER.map(phaseLabel), normalise: "phase" }, { key: "drugs", label: "Product", width: "w-48" }, { key: "sponsor", label: "Sponsor", width: "w-48" }],
      columns: [{ key: "phase", label: "Phase", sortable: true, hide: "hidden sm:table-cell" }, { key: "drugs", label: "Products", hide: "hidden md:table-cell" }, { key: "indications", label: "Indications", hide: "hidden lg:table-cell" }, { key: "sponsor", label: "Sponsor", hide: "hidden lg:table-cell" }, { key: "year", label: "Reported", sortable: true, numeric: true }],
    ...
    case "pairing": {
      ...
      return {
        hideStatus: true,
        rows: g.kind("pairing").map((p) => ({ ...base(p), ...firstOf(techVisual(g, p.a), techVisual(g, p.b), borrowed(p)), facets: { type: [typeLabel(p.pairingType)], indications: names(p.indications) }, cols: { type: fl("type", typeLabel(p.pairingType)), a: links([p.a]), b: links([p.b]), indications: links(p.indications) }, sortKeys: { type: rank(ORDER, p.pairingType) } })),
        facets: [{ key: "type", label: "Type", searchable: false, width: "w-52", order: ORDER.map(typeLabel) }, { key: "indications", label: "Indication", width: "w-52" }],
        columns: [{ key: "type", label: "Type", sortable: true, chip: true, tip: "Combination: given together. Sequence: one after the other. Diagnostic → therapeutic: a test that picks the treatment. Platform: a shared technology. Caution: a pairing to avoid." }, { key: "a", label: "First", hide: "hidden md:table-cell" }, { key: "b", label: "Second", hide: "hidden md:table-cell" }, { key: "indications", label: "Indications", hide: "hidden lg:table-cell" }],
        defaultSort: { key: "type", dir: 1 },
      };
    }
    case "roadmap": return {
      hideStatus: true,
      rows: g.kind("roadmap").map((r) => ({ ...base(r), ...borrowed(r), facets: { indications: names(r.indications) }, cols: { indications: links(r.indications), steps: count(r.steps.length, r, "steps", "step", "of"), current: r.steps.filter((s) => s.status === "current").length, emerging: r.steps.filter((s) => s.status === "emerging").length }, sortKeys: { steps: r.steps.length } })),
      facets: [{ key: "indications", label: "Indication", width: "w-52" }],
      columns: [{ key: "indications", label: "Indications", hide: "hidden md:table-cell" }, { key: "steps", label: "Steps", sortable: true, numeric: true }, { key: "current", label: "Current", numeric: true, hide: "hidden sm:table-cell" }, { key: "emerging", label: "Emerging", numeric: true, hide: "hidden sm:table-cell" }],
    };
    case "idea": {
    ...
    case "paper": return {
      hideStatus: true,
      rows: g.kind("paper").map((p) => ({ ...base(p), logo: journalLogo(g, p.journal), sub: `${p.authors} · ${p.journal} ${p.year}`, facets: { type: [cap(p.paperType.replace(/-/g, " "))], year: [String(p.year)], journal: [p.journal], indications: names(p.indications), changed: [p.changedPractice ? "Changed practice" : "Did not (yet)"] }, cols: { changed: fl("changed", p.changedPractice ? "Changed practice" : "Did not (yet)"), type: fl("type", cap(p.paperType.replace(/-/g, " "))), year: fl("year", p.year), journal: journalCell(p.journal), indications: links(p.indications), drugs: links(p.drugs.slice(0, 3)) }, sortKeys: { year: p.year, changed: p.changedPractice ? 0 : 1 }, tie: p.year })),
      facets: [{ key: "type", label: "Type", searchable: false, width: "w-44" }, { key: "indications", label: "Indication", width: "w-52" }, { key: "journal", label: "Journal", width: "w-52" }, { key: "year", label: "Year", searchable: false, width: "w-32" }, { key: "changed", label: "Practice", searchable: false, width: "w-44", order: ["Changed practice", "Did not (yet)"] }],
      columns: [{ key: "changed", label: "Practice", sortable: true, tip: "Whether the paper changed what clinicians do: guidelines, approvals, or the standard of care." }, { key: "type", label: "Type", sortable: true, chip: true }, { key: "journal", label: "Journal", sortable: true, hide: "hidden md:table-cell" }, { key: "year", label: "Year", sortable: true, numeric: true }, { key: "indications", label: "Indications", hide: "hidden lg:table-cell" }, { key: "drugs", label: "Products", hide: "hidden xl:table-cell" }],
    ...
    case "person": return {
      hideStatus: true,
      rows: g.kind("person").map((p) => { const inst = p.institutionId ? g.get(p.institutionId) : undefined; return { ...base(p), logo: portraitSrc(p.id), round: true, avatar: "person", sub: `${p.role}${inst ? ` · ${inst.name}` : ""}`, facets: { specialism: p.specialisms, institution: inst ? [inst.name] : [], indications: names(p.indications), country: inst && inst.kind === "institution" ? [inst.country] : [], role: p.tags.filter((t) => t in HERO_ROLE_LABEL).map((t) => HERO_ROLE_LABEL[t]) }, cols: { institution: inst ? [link(inst, inst.name)] : undefined, specialisms: p.specialisms.map((s) => ({ facet: "specialism", value: s })), papers: count(p.papers.length, p, "papers", "paper", "by"), trials: count(p.trials.length, p, "connected", "trial", "linked to") }, sortKeys: { papers: p.papers.length, trials: p.trials.length } }; }),
      facets: [{ key: "role", label: "Heroes", searchable: false, width: "w-40", order: Object.values(HERO_ROLE_LABEL) }, { key: "specialism", label: "Specialism", width: "w-56" }, { key: "institution", label: "Institution", width: "w-56" }, { key: "indications", label: "Indication", width: "w-52" }, { key: "country", label: "Country", searchable: false, width: "w-40" }],
...
function termRowVisual(v: TermVisual, category: string): Partial<BrowserRow> {
  switch (v.kind) {
    ...
    case "indication": return { cancerIcon: v.indicationId };

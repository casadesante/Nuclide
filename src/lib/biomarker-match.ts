...
export type MatchRow = {
  ...
  /** Indication ids the entity is relevant to (direct or via its drugs). */
  indications: string[];
...
export function scoreRows(rows: MatchRow[], selected: Biomarker[], indicationId: string | null): Scored[] {
  const out: Scored[] = [];
  for (const row of rows) {
    ...
    if (indicationId && row.indications.includes(indicationId)) score += 2;

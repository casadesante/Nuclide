...
export type Score = { verdict: Verdict; reasons: string[]; checklist: Check[]; /** Criteria with nothing Nuclide can assess; only the team can check these. */ unassessed: number };
...
/** Canonical biomarker names and the patterns that name them in criteria text or Nuclide biomarker labels. */
...
/** Therapy classes and agents recognised in criteria text and in Nuclide drug names. */
...
/** Canonical biomarker names present (and explicitly absent) in the reader's profile, derived from Nuclide biomarker labels. */
...
/** Therapy classes present in the reader's prior lines, derived from Nuclide drug names (falling back to the id). */
...
export function score(criteria: Criterion[], profile: ScoreProfile, ctx: ScoreContext): Score {
  ...
  for (const c of criteria) {
    ...
    if (tg.ecog) checks.push(["unknown", `Needs a performance status of ${tg.ecog.length === 1 ? `ECOG ${tg.ecog[0]}` : `ECOG ${tg.ecog[0]} to ${tg.ecog[tg.ecog.length - 1]}`}. Nuclide does not record fitness; your team scores this in clinic.`]);
    ...
    if (tg.brainMets) {
      ...
      checks.push(["unknown", `${msg} Nuclide does not record where the cancer has spread; your team will know.`]);
    }
    ...
    if (tg.age) {
      ...
      checks.push(["unknown", `Age ${range}. Nuclide does not ask your age.`]);
    }
  ...
  if (unmet.length) {
    ...
    verdict = "unclear";
    reasons.push(checklist.length ? `Nothing in your profile contradicts the criteria, but ${requiredBmUnknown ? "a required biomarker is not in your profile and " : ""}${unknown.length + unassessed} criteria need information Nuclide does not have.` : "The criteria could not be matched to anything in your profile.");
  }

...
const NUCLIDE = "(cancer OR tumor OR tumour OR oncology OR carcinoma OR lymphoma OR leukemia OR leukaemia OR myeloma OR sarcoma OR melanoma OR glioma)";
...
export const TECH_QUERIES: Record<string, string> = {
  adc: `${inTitleAbstract(["antibody-drug conjugate", "antibody drug conjugate", "antibody-drug conjugates"])} AND ${NUCLIDE}`,
  ...
  "immune-stimulating-adc": `${inTitleAbstract(["immune-stimulating antibody conjugate", "ISAC", "TLR agonist antibody conjugate"])} AND ${NUCLIDE}`,
  ...
  "t-cell-engager": `${inTitleAbstract(["bispecific T-cell engager", "T cell engager", "CD3 bispecific"])} AND ${NUCLIDE}`,
  ...
  "cytokine-therapy": `${inTitleAbstract(["engineered cytokine", "IL-2 variant", "IL-15 superagonist"])} AND ${NUCLIDE}`,
  "sting-agonist": `${inTitleAbstract(["STING agonist", "cGAS-STING"])} AND ${NUCLIDE}`,
  ...
  mri: `${inTitleAbstract(["magnetic resonance imaging", "multiparametric MRI"])} AND ${NUCLIDE}`,
  ultrasound: `${inTitleAbstract(["ultrasound", "endoscopic ultrasound"])} AND ${NUCLIDE} AND (diagnosis OR biopsy OR screening)`,
  mammography: `${inTitleAbstract(["mammography", "digital breast tomosynthesis", "breast cancer screening"])}`,
  "radiology-ai-screening": `${inTitleAbstract(["artificial intelligence", "deep learning"])} AND ${inTitleAbstract(["mammography", "lung cancer screening", "radiology"])} AND ${NUCLIDE}`,
  ...
  "pathology-foundation-model": `${inTitleAbstract(["foundation model"])} AND ${inTitleAbstract(["pathology", "histopathology", "radiology"])} AND ${NUCLIDE}`,
  "ai-drug-design": `${inTitleAbstract(["AI drug discovery", "generative model", "de novo design"])} AND ${NUCLIDE}`,
  "ai-trial-matching": `${inTitleAbstract(["clinical trial matching", "trial eligibility", "large language model"])} AND ${NUCLIDE}`,
  "liquid-biopsy": `${inTitleAbstract(["liquid biopsy", "circulating tumor DNA", "cell-free DNA"])} AND ${NUCLIDE}`,
  "mrd-testing": `${inTitleAbstract(["minimal residual disease", "molecular residual disease", "ctDNA MRD"])} AND ${NUCLIDE}`,
  ...
  "wes-wgs": `${inTitleAbstract(["whole-genome sequencing", "whole genome sequencing", "whole-exome sequencing"])} AND ${NUCLIDE}`,
  "rna-seq": `${inTitleAbstract(["RNA sequencing", "transcriptomic profiling", "gene expression signature"])} AND ${NUCLIDE}`,
  "single-cell-spatial": `${inTitleAbstract(["single-cell RNA sequencing", "spatial transcriptomics", "single-cell"])} AND ${NUCLIDE}`,
  proteomics: `${inTitleAbstract(["proteomics", "proteogenomic", "phosphoproteomics"])} AND ${NUCLIDE}`,
  "methylation-profiling": `${inTitleAbstract(["DNA methylation", "methylation classifier", "cfDNA methylation"])} AND ${NUCLIDE}`,
  "histopathology-ihc": `${inTitleAbstract(["immunohistochemistry"])} AND ${NUCLIDE} AND (biomarker OR scoring)`,
  ...
  "functional-drug-testing": `${inTitleAbstract(["functional precision medicine", "ex vivo drug sensitivity", "drug sensitivity testing"])} AND ${NUCLIDE}`,
  ...
  "crispr-screens": `${inTitleAbstract(["CRISPR screen", "genome-wide CRISPR", "CRISPR-Cas9 screen"])} AND ${NUCLIDE}`,
  "synthetic-lethality-approaches": `${inTitleAbstract(["synthetic lethality", "synthetic lethal"])} AND ${NUCLIDE}`,
  "robotic-surgery": `${inTitleAbstract(["robotic surgery", "robot-assisted surgery", "robotic-assisted"])} AND ${NUCLIDE}`,
  "fluorescence-guided-surgery": `${inTitleAbstract(["fluorescence-guided surgery", "fluorescence guided surgery", "near-infrared fluorescence"])} AND ${NUCLIDE}`,
  "sentinel-node": `${inTitleAbstract(["sentinel lymph node biopsy", "sentinel node"])} AND ${NUCLIDE}`,
  "thermal-ablation": `${inTitleAbstract(["radiofrequency ablation", "microwave ablation", "cryoablation"])} AND ${NUCLIDE}`,
  "hifu-histotripsy": `${inTitleAbstract(["high-intensity focused ultrasound", "histotripsy", "focused ultrasound"])} AND ${NUCLIDE}`,
  ...
  "cytotoxic-chemotherapy": `${inTitleAbstract(["chemotherapy"])} AND ${NUCLIDE} AND (randomized OR phase 3 OR phase III)`,
  platinum: `${inTitleAbstract(["carboplatin", "cisplatin", "platinum-based chemotherapy"])} AND ${NUCLIDE}`,
  hipec: `${inTitleAbstract(["hyperthermic intraperitoneal chemotherapy", "HIPEC", "PIPAC"])}`,
  "kinase-inhibitors": `${inTitleAbstract(["tyrosine kinase inhibitor", "kinase inhibitor"])} AND ${NUCLIDE}`,
  "monoclonal-antibody": `${inTitleAbstract(["monoclonal antibody"])} AND ${NUCLIDE} AND (phase 3 OR phase III OR approval)`,
  "bispecific-antibody": `${inTitleAbstract(["bispecific antibody", "bispecific antibodies"])} AND ${NUCLIDE}`,
  ...
  "protac-degrader": `${inTitleAbstract(["PROTAC", "targeted protein degradation", "molecular glue degrader"])} AND ${NUCLIDE}`,
  "antisense-sirna": `${inTitleAbstract(["antisense oligonucleotide", "siRNA"])} AND ${NUCLIDE}`,
  antiangiogenic: `${inTitleAbstract(["anti-angiogenic", "antiangiogenic", "VEGF inhibitor", "bevacizumab"])} AND ${NUCLIDE}`,
  ...
  "epigenetic-drugs": `${inTitleAbstract(["epigenetic therapy", "HDAC inhibitor", "EZH2 inhibitor", "menin inhibitor", "IDH inhibitor"])} AND ${NUCLIDE}`,
  ttfields: `${inTitleAbstract(["tumor treating fields", "tumour treating fields", "TTFields"])}`,
  photoimmunotherapy: `${inTitleAbstract(["photoimmunotherapy", "photodynamic therapy"])} AND ${NUCLIDE}`,
  hyperthermia: `${inTitleAbstract(["hyperthermia"])} AND ${NUCLIDE} AND (radiotherapy OR chemotherapy)`,
  "scalp-cooling": `${inTitleAbstract(["scalp cooling", "chemotherapy-induced alopecia"])}`,
  "cardio-oncology": `${inTitleAbstract(["cardio-oncology", "cardiotoxicity"])} AND ${NUCLIDE}`,
  "exercise-oncology": `${inTitleAbstract(["exercise oncology", "exercise intervention"])} AND ${NUCLIDE} AND survival`,
  "geriatric-assessment": `${inTitleAbstract(["geriatric assessment"])} AND ${NUCLIDE}`,
  ...
  "optical-imaging": `${inTitleAbstract(["optical imaging", "fluorescence imaging", "indocyanine green"])} AND ${NUCLIDE}`,
  "whole-body-mri": `${inTitleAbstract(["whole-body MRI", "whole body MRI"])} AND ${NUCLIDE}`,

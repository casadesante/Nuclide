...
export function fieldText(e: Entity): string {
  switch (e.kind) {
    case "indication": return [e.group, e.burden ?? "", ...e.subtypes, ...e.biomarkers, ...e.standardOfCare.map((s) => `${s.setting} ${s.approach}`), ...e.stateOfArt, ...e.openProblems, e.prognosis?.text ?? ""].join(" ");

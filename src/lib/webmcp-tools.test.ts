...
beforeEach(() => {
  vi.stubGlobal("window", { location: { origin: "https://nuclide.example" } });
...
describe("WebMCP public data tools", () => {
  it("filters before limiting, reuses search, and includes attribution", async () => {
    ...
    expect(result).toMatchObject({ data: { results: [{ id: "drug-a", url: "https://nuclide.example/drugs/drug-a/" }] }, attribution: expect.stringContaining("CC BY-NC 4.0"), disclaimer: expect.stringContaining("not medical advice") });
  });
  ...
  it("returns the complete record with sources and neighbours", async () => {
    ...
    expect(await entityTool.execute({ id: "her2" })).toMatchObject({ data: { ...record, found: true, url: "https://nuclide.example/targets/her2/" } });

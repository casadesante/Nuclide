...
export function EntityBrowser({ rows, facets, columns, noun, defaultSort, hideStatus = false, hideTldr = false, external = null, onExternalChange, nameKind }: {
  ...
  // synchronously). Values repeat (`?indications=A&indications=B`) or are comma-separated; a raw value that is itself a
  ...
  const tableCols: Column<BrowserRow>[] = [
    { key: "name", label: t("name"), sortable: true, render: (r) => (
      <div className="min-w-[220px] flex items-start gap-2">
        ...
        {r.cancerIcon && <Link href={r.route} aria-label={`${r.name} organ icon`} className="inline-flex h-10 w-14 shrink-0 items-center justify-center rounded-md border border-border bg-accent-soft text-accent"><IndicationIcon indicationId={r.cancerIcon} className="h-7 w-7" /></Link>}

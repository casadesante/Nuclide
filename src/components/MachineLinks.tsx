...
const stamp = (e: Entity, format: string) => ({ "data-nuclide-id": e.id, "data-nuclide-kind": e.kind, "data-nuclide-format": format });
 ...
 *
 * Also stamps `data-nuclide-id` / `data-nuclide-kind` on `<main>`: the inline script does it in the static HTML (it runs
...
export function MachineLinks({ e }: { e: Entity }) {
  ...
  const setMain = `var m=document.getElementById("main");if(m){m.setAttribute("data-nuclide-id",${JSON.stringify(e.id)});m.setAttribute("data-nuclide-kind",${JSON.stringify(e.kind)})}`.replace(/</g, "\\u003c");
  return (
    <nav aria-label="Machine-readable versions" className="sr-only" data-nuclide-id={e.id} data-nuclide-kind={e.kind} data-nuclide-route={route}>
      ...
      <p data-nuclide-mcp={MACHINE.mcp.command}>
      ...
      <p data-nuclide-webmcp={MACHINE.webmcp.tools.join(" ")}>WebMCP tools registered on this page: {MACHINE.webmcp.tools.join(", ")}.</p>

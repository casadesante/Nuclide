...
const SITE = "nuclide-umber.vercel.app";
const LICENCE = "CC BY-NC 4.0, attribute \"Data from Nuclide (nuclide.cc)\"; commercial use needs a licence";
...
export function ChartExport({ children, title, source, filename, className = "", align = "end" }: { children: ReactNode; title: string; source?: string; filename?: string; className?: string; align?: "start" | "end" }) {
  ...
  const footer = `${title} · Source: ${source ?? "Nuclide corpus"} · ${SITE} · ${LICENCE} · ${new Date().toISOString().slice(0, 10)}`;

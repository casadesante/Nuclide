...
export function PowerView({ rows, indications, initialCancer, initialKind }: { rows: PowerRow[]; indications: PowerCancer[]; initialCancer?: string; initialKind?: Kind }) {
  ...
  useEffect(() => {
    ...
    const id = requestAnimationFrame(() => {
      ...
      const c = p.get("indication"), k = p.get("kind") as Kind | null, query = p.get("q");
      if (c && indications.some((x) => x.id === c)) setCancer(c);
  ...
  useEffect(() => {
    ...
    if (cancer) p.set("indication", cancer);
  ...
  const togglePersonalise = () => {
    ...
    setPersonalise(next);
    if (next && !cancer && profile.indicationId && indications.some((c) => c.id === profile.indicationId)) setCancer(profile.indicationId);
  };

  const cancerOptions = useMemo(() => indications.map((c) => ({ value: c.id, label: c.name, group: c.group })), [indications]);
  const cancerName = indications.find((c) => c.id === cancer)?.name;
  ...
  return (
    <div>
      <Toolbar
        ...
        left={<>
          <FacetSelect label="Cancer" options={cancerOptions} value={cancer} onChange={(v) => setCancer(v as string | null)} allLabel="All indications" width="w-72" />
        ...
        right={cancer ? <Link href={indications.find((c) => c.id === cancer)!.route} className="underline">cancer page →</Link> : undefined}
      />
      ...
      <details className="mt-3 text-xs text-muted">
        ...
        <p className="mt-1 max-w-3xl">For a chosen indication: standard-of-care mention +{WEIGHTS.soc}, in its pipeline +{WEIGHTS.pipeline}, in its history +{WEIGHTS.history}, directly linked +{WEIGHTS.direct}, linked through one of its drugs +{WEIGHTS.indirect}. Always: evidence tier (approved 10 … concept 0) plus half a point per connection in the graph, capped at 10. With Personalise on: a match to one of your biomarkers +{PERSONAL_WEIGHTS.biomarker}, approval in your region +{PERSONAL_WEIGHTS.region}, a standard-of-care setting matching your stage +{PERSONAL_WEIGHTS.stage}. It ranks documentation and evidence, not clinical benefit.</p>
      </details>
    </div>
  );
}

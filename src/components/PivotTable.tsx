...
export type Dim = "indication" | "target" | "modality" | "company" | "front" | "status" | "phase";
export type DimMeta = Record<Dim, { label: string; ids?: Record<string, string>; /** Page for a header label, when the dimension is made of records (indications, targets, companies). */ routes?: Record<string, string> }>;

const DIMS: Dim[] = ["indication", "target", "modality", "company", "front", "status", "phase"];
...
export function PivotTable({ facts, meta }: { facts: Fact[]; meta: DimMeta }) {
  ...
  const [rowDim, setRowDim] = useState<Dim>("indication");
  ...
  const linkFor = (r: string, c: string) => {
    ...
    const indicationId = rowDim === "indication" ? meta.indication.ids?.[r] : colDim === "indication" ? meta.indication.ids?.[c] : undefined;
    return indicationId ? `/explore/?cancer=${indicationId}&kind=${kind}` : `/${kindRoute}/`;
  };
  ...
  return (
    <div>
      ...
      <div className="card overflow-x-auto">
        <table className="nuclide text-xs">
          <thead>
            <tr>

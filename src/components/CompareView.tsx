...
export type CompareKind = "drug" | "technology" | "target" | "trial" | "indication";
...
const KINDS: CompareKind[] = ["drug", "technology", "target", "trial", "indication"];
const DEFAULTS: Record<CompareKind, string[]> = {
  ...
  indication: ["tnbc", "breast-hr-positive"],
};
const MAX = 5;
...
export function CompareView({ items }: { items: CompareItem[] }) {
  ...
  return (
    <div>
      ...
      <p className="text-xs text-muted mt-3">Rows marked • differ between the selected items. Fields come from the Nuclide records; an empty field shows a dash. To fill a gap, use the suggest-an-edit form on that record&apos;s page.</p>
    </div>
  );
}

...
/**
 * Wireframe organ schematic for a indication: the organ, its subsites (hot rings) and the regional node
 * stations (accent clusters), with a legend that links each subsite to the cancer's recorded subtypes.
 * Compact mode is a bare thumbnail for the atlas grid. Renders nothing for indications without an organ drawing.
 */
export function OrganSchematic({ indicationId, organ, compact = false, height }: { indicationId?: string; organ?: Organ; compact?: boolean; height?: string }) {
  const o = organ ?? (indicationId ? organFor(indicationId) : undefined);
  if (!o) return null;
  const mesh = organMesh(o);
  if (compact) return <Wireframe3D mesh={mesh} height={height ?? "h-40"} speed={0.2} compact />;
  const g = graph();
  const cancer = indicationId ? g.get(indicationId) : undefined;
  const subtypes = cancer && cancer.kind === "indication" ? cancer.subtypes : [];
  const others = o.indications.filter((id) => id !== indicationId).map((id) => g.get(id)).filter((c): c is NonNullable<typeof c> => !!c);
  return (

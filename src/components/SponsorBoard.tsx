...
export function SponsorBoard({ rows, indications }: { rows: SponsorRow[]; indications: Array<{ id: string; name: string }> }) {
  const [cancer, setCancer] = useState<string | null>(null);
  const [kinds, setKinds] = useState<string[]>([]);
  const [q, setQ] = useState("");
  const [sort, setSort] = useState<SortState>({ key: "total", dir: -1 });
  const cancerName = useMemo(() => new Map(indications.map((c) => [c.id, c.name])), [indications]);

  const filtered = useMemo(() => {
    const n = q.trim().toLowerCase();
    const list = rows.filter((r) => (!kinds.length || kinds.includes(r.kind)) && (!cancer || (r.indications[cancer] ?? 0) > 0) && (!n || r.label.toLowerCase().includes(n)));
    const inCancer = (r: SponsorRow) => (cancer ? r.indications[cancer] ?? 0 : r.registryTotal + r.corpusTrials.length);
  ...
  const cancerOptions = useMemo(() => { const m = new Map<string, number>(); for (const r of rows) for (const [c, n] of Object.entries(r.indications)) m.set(c, (m.get(c) ?? 0) + n); return [...m.entries()].sort((a, b) => b[1] - a[1]).map(([value, count]) => ({ value, label: cancerName.get(value) ?? value, count })); }, [rows, cancerName]);
  ...
  const columns: Column<SponsorRow>[] = [
    ...
    { key: "indications", label: "Top indications", hide: "hidden lg:table-cell", render: (r) => <div className="flex flex-wrap gap-1">{Object.entries(r.indications).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([c, n]) => <button key={c} type="button" onClick={() => setCancer(c)} className={`chip border ${KIND_COLOR.indication}`}>{cancerName.get(c) ?? c} {n}</button>)}</div> },
  ];

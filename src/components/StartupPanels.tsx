      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {rounds.length > 0 && (
          <div className="card p-4 text-sm">
            ...
            <table className="nuclide text-sm">
...
/** Investor pages: the companies in Nuclide that name this investor, derived from backlinks. */
            ...
            <tbody>{portfolio.map((c) => {
              ...
              return (
                <tr key={c.id}>
                  ...
                  <td className="hidden md:table-cell text-xs">{c.indications.map((id) => g.get(id)).filter((x) => !!x).map((x) => <Link key={x.id} href={routeFor(x)} className="underline mr-2">{x.name}</Link>)}</td>

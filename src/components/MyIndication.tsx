 ...
 * fetches from /api/v1/my-indications.json only once a cancer is remembered, so none of these imports the graph and no
 * page carries the list in its payload. Until storage and the list have been read they render their unset state,
 * which is also what the server rendered, so hydration is clean. MyIndicationPin still takes the list as a prop because
...
/** Indication hub: the chosen cancer pinned above the grouped grid, with a link to change it. Renders nothing when unset. */
export function MyIndicationPin({ indications }: { indications: MyCancerTile[] }) {
  const { id } = useMyCancer();
  const mine = pickMyCancer(indications, id);
  if (!mine) return null;
  return (
    <div className="mb-8">
      ...
      <div className="flex flex-wrap items-stretch gap-2">
        ...
        <div className="flex flex-col justify-center gap-1.5 text-sm">
          ...
          <Link href={`/trials/?indications=${encodeURIComponent(mine.name)}`} className="chip border border-border bg-card hover:bg-foreground/5"><TrialGlyph className="h-3 w-3" />Trials</Link>
...
/** Trials index: one tap to the table filtered to the chosen cancer (the browser reads `?indications=` on mount). */

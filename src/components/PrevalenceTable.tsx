...
type Row = { indicationId: string; pct: number | string; measure?: string; source?: string; note?: string };
  ...
  return (
    <div className="card overflow-x-auto">
      <table className="nuclide">
        ...
        <tbody>
          {rows.map((r, i) => { const c = g.get(r.indicationId); return (
            <tr key={i}>
              <td>{c ? <Link href={routeFor(c)} className="font-medium hover:underline">{c.name.replace(/ \(.*\)$/, "")}</Link> : r.indicationId}</td>
              <td><Bar pct={r.pct} /></td>
...
/** Per indication: which targets are present and how often. */
export function IndicationPrevalence({ indicationId }: { indicationId: string }) {
  const g = graph();
  const rows: Array<{ target: Target; row: Row }> = [];
  for (const t of g.kind("target")) for (const r of t.prevalence) if (r.indicationId === indicationId) rows.push({ target: t, row: r });

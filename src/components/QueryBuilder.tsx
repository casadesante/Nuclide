...
const EXAMPLES: SavedQuery[] = [
  ...
  { title: "Cancers with no phase-3 product in their graph", kind: "indication", clauses: [{ type: "linked-kind", not: true, kind: "drug", status: "phase-3" }] },
...
const TEXT_EXAMPLES: Array<{ title: string; q: string }> = [
  { title: "Targets of approved TNBC products", q: "indication:tnbc -> drugs(status:approved) -> targets" },
  { title: "Companies behind phase-3 products for lung cancer", q: "indication:nsclc -> drugs(status:phase-3) -> companies" },
    ...
    <div className="grid gap-8 lg:grid-cols-[300px_1fr] [&>*]:min-w-0">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3 text-sm">
          ...
          {mode === "form" && <span className="text-xs text-muted">Text mode adds hops: <span className="font-mono">indication:tnbc -&gt; drugs -&gt; targets</span></span>}
        </div>
          ...
          <div className="card p-4 mb-4 space-y-2">
            <div className="relative">
              <input ref={input} value={text} onChange={(e) => { setText(e.target.value); setShowSuggest(true); }} onFocus={() => setShowSuggest(true)} onBlur={() => setTimeout(() => setShowSuggest(false), 120)} onKeyDown={(e) => { if (e.key === "Escape") setShowSuggest(false); if (e.key === "Tab" && suggestions.options[0]) { e.preventDefault(); accept(suggestions.options[0].value); } }}
                spellCheck={false} aria-label="Graph query" placeholder="indication:tnbc -> drugs(status:approved) -> targets" className="w-full rounded-lg border border-border bg-card px-3 py-2.5 font-mono text-sm outline-none focus:ring-2 focus:ring-accent/40" />

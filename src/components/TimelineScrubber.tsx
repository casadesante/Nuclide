...
export type TLTrial = { year: number; name: string; route: string; status?: string; indications: string[] };
export type TLHistory = { year: number; indication: string; cancerRoute: string; title: string; note?: string; refs: Array<{ name: string; route: string }> };
...
export function TimelineScrubber({ data }: { data: TimelineData }) {
  ...
  const socAsOf = useMemo(() => {
    ...
    for (const h of data.history) if (h.year <= year && h.refs.length && (!byCancer.has(h.indication) || byCancer.get(h.indication)!.year <= h.year)) byCancer.set(h.indication, h);
    return [...byCancer.values()].sort((a, b) => a.indication.localeCompare(b.indication));
  }, [data, year]);
    ...
    <div>
      ...
      <div className="grid gap-8 lg:grid-cols-2 mt-8">
        <section>
          ...
          {trialsThisYear.length > 0 && <Block title={`Trials reported (${trialsThisYear.length})`}>{trialsThisYear.map((t) => <li key={t.route}><Link href={t.route} className="font-medium hover:underline">{t.name}</Link> {t.status && <span className={`chip ${statusClass(t.status)}`}>{STATUS_LABEL[t.status] ?? t.status}</span>} <span className="text-muted">· {t.indications.join(", ")}</span></li>)}</Block>}
          {historyThisYear.length > 0 && <Block title={`History events (${historyThisYear.length})`}>{historyThisYear.map((h, i) => <li key={i}><Link href={h.cancerRoute} className="text-muted hover:underline">{h.indication}</Link>: <span className="font-medium">{h.title}</span>{h.note && <span className="text-muted"> · {h.note}</span>}{h.refs.length > 0 && <span className="ml-1">{h.refs.map((r) => <Link key={r.route} href={r.route} className="chip bg-foreground/5 mr-1 hover:underline">{r.name}</Link>)}</span>}</li>)}</Block>}
        ...
        <section>
          ...
          <ul className="space-y-2 text-sm">
            {socAsOf.map((h) => <li key={h.indication} className="card p-3"><Link href={h.cancerRoute} className="font-medium hover:underline">{h.indication}</Link> <span className="text-muted">({h.year}) · </span>{h.title}<div className="mt-1 flex flex-wrap gap-1">{h.refs.map((r) => <Link key={r.route} href={r.route} className="chip bg-foreground/5 hover:underline">{r.name}</Link>)}</div></li>)}

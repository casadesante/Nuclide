...
const PAGES: Item[] = [
  ...
  { id: "p-compare", name: "Compare products, technologies, targets, trials, or indications", tldr: "Up to five side by side, differences highlighted", route: "/compare/", action: true },
...
const GOTO: Array<{ key: string; route: string; label: string }> = [
  ...
  { key: "c", route: "/indications/", label: "Indications" },
...
const SHORTCUTS_KEY = "nuclide:shortcuts:v1";
...
export function CommandPalette() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("nuclide:open-palette", onOpen);
    window.addEventListener("nuclide:open-shortcuts", onSheet);
    return () => { window.removeEventListener("keydown", onKey); window.removeEventListener("nuclide:open-palette", onOpen); window.removeEventListener("nuclide:open-shortcuts", onSheet); };
  }, [open, sheet, router]);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center bg-black/40 backdrop-blur-sm p-4 pt-[12vh]" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
      <div ref={dialog} role="dialog" aria-modal="true" aria-label="Search Nuclide" onKeyDown={trap} className="w-full max-w-2xl card shadow-2xl overflow-hidden">
        <div className="flex items-center gap-3 px-4 border-b border-border">
          ...
          <input ref={input} autoFocus value={q} onChange={(e) => { setQ(e.target.value); run(e.target.value); }} onKeyDown={onKeyDown}
            placeholder="Search products, targets, indications, trials, companies, pages…" className="flex-1 bg-transparent py-3.5 text-base outline-none"
...
export function PaletteTrigger({ className = "" }: { className?: string }) {
  ...
  return (
    <button type="button" onClick={() => window.dispatchEvent(new Event("nuclide:open-palette"))} className={`ctl ctl-icon text-muted min-w-0 overflow-hidden ${className}`} aria-label={t("header.openSearch")}>

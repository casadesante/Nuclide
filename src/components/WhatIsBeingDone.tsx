...
type TopicMeta = {
  ...
  keywords: RegExp;
  /** Pages on Nuclide that go deeper. */
...
export const TOPICS: Record<ProblemTopic, TopicMeta> = {
  recurrence: {
    ...
    pages: [{ label: "Indications", href: "/indications/" }],
  },
  spread: {
  ...
  resistance: {
    ...
    pages: [{ label: "Open problems", href: "/gaps/" }],
  },
  "side-effects": {
    ...
    pages: [{ label: "Dosimetry", href: "/roadmaps/dosimetry-roadmap/" }],
    skipBottleneckRelations: true,
  },
  "late-diagnosis": {
    label: "Finding cancer earlier",
    frame: "Stage at diagnosis is the single biggest lever on outcomes, and PET tracers built to find disease early or catch recurrence before symptoms are moving it.",
    bottlenecks: [],
    seeds: ["psma-pet", "sstr-pet", "caix-pet", "fapi-pet", "her2-pet", "mibg-theranostics", "fdg-pet", "total-body-pet-screening", "f-18-fet", "fluciclovine-f18", "f-18-fdopa", "f-18-fluorothymidine"],
    ...
    pages: [{ label: "Roadmaps", href: "/roadmaps/" }],
  },
  access: {
    ...
    pages: [{ label: "Approvals and coverage by region", href: "/regulatory/regions/" }],
    skipBottleneckRelations: true,
  },
  rare: {
    label: "Rare indications and small trials",
    frame: "Tumour-agnostic approvals, basket trials, reference centres and international cooperative groups are how rare indications get evidence.",
    bottlenecks: ["b-rare-indications", "b-trial-enrolment"],
  ...
  "no-target": {
    ...
    pages: [{ label: "Targets", href: "/targets/" }, { label: "Radiopharma roadmap", href: "/roadmaps/radiopharma-roadmap/" }],
  },
};
...
export function assembleTopic(topic: ProblemTopic, indicationId?: string): Assembled | null {
  const g = graph();
  ...
  if (indicationId && g.get(indicationId)) {
    for (const [kind, list] of g.neighbours(indicationId)) {
      for (const e of list) {
        linked.add(e.id);
  ...
  const relevant = (e: Entity) => !indicationId || linked.has(e.id) || e.indications.includes(indicationId);
 ...
 * `indicationId` limits the lists to work directly linked to that cancer or explicitly tagged for it. Renders nothing when no topic matches
 * or the corpus has nothing on it.
 */
export function WhatIsBeingDone({ topic, indicationId, compact = false, className = "" }: { topic: ProblemTopic | string; indicationId?: string; compact?: boolean; className?: string }) {
  const id = topicFor(topic);
  if (!id) return null;
  const data = assembleTopic(id, indicationId);
  if (!data) return null;
    ...
    <div className={`card p-4 text-sm ${className}`}>
      ...
      {(data.background.length > 0 || meta.pages.length > 0) && (
        <p className="text-xs text-muted mt-3 leading-relaxed">
          ...
          {meta.pages.length > 0 && <>Also on Nuclide: {meta.pages.map((p, i) => <span key={p.href}>{i > 0 && " · "}<Link className="underline" href={p.href}>{p.label}</Link></span>)}.</>}
        </p>
      )}
    </div>
  );
...
export function WhatIsBeingDoneFor({ text, indicationId, max = 1 }: { text: string; indicationId?: string; max?: number }) {
  const topics = topicsForText(text, max);
  if (!topics.length) return null;
  return <div className="space-y-3">{topics.map((t) => <WhatIsBeingDone key={t} topic={t} indicationId={indicationId} compact />)}</div>;
}

 ...
 * Nuclide objects its title names. Filter by source or show only items that touch something in the corpus.
 */
    ...
    <div>
      <Toolbar count={filtered.length} total={snap.items.length} noun="items"
        left={<>
          ...
          <label className="inline-flex items-center gap-1.5 text-sm"><input type="checkbox" checked={matchedOnly} onChange={(e) => setMatchedOnly(e.target.checked)} /> Only items naming an Nuclide object</label>

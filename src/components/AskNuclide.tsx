...
/**
 * Ask Nuclide. In the browser: the question is read for its intent and the records it names (name and alias
 ...
 */
export function AskNuclide({ examples }: { examples: AskExample[] }) {
    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_320px] [&>*]:min-w-0">
        ...
        {r && (
          <div className="mt-6" aria-live="polite">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <span className="kicker">Answer from Nuclide records</span>
            ...
            ) : <p className="text-sm text-muted">Nothing in Nuclide matches that question. Try naming a cancer, product, target, trial or term.</p>}
            {r.sources.length > 0 && (
      ...
      <aside className="space-y-4">
        ...
        <div className="card p-3 text-xs text-muted space-y-2">
          <p><strong className="text-foreground/80">How it works.</strong> Your question is read for what it asks (what something means, how a cancer is treated, whether a product is approved, how it works, side effects, trials, results, a comparison, outlook, who or where, cost) and for the records it names, matched against every Nuclide name and alias. Those records are fetched and a template for that kind of question fills the answer from their fields, plain English first, one citation per sentence. Approval answers start with your chosen region.</p>
          <p><strong className="text-foreground/80">What it cannot do.</strong> It does not reason, weigh evidence or fill gaps. If Nuclide has no record on the point, it says so and falls back to the closest sentences from word and concept search. The <Link href="/eval/" className="underline">open benchmark</Link> scores this method in public.</p>
        </div>
      </aside>
    </div>
  );

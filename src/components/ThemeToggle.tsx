 ...
 * the saved theme before paint. `data-nuclide-toggle` lets the account menu's preference chip press this control.
 */
export function ThemeToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button type="button" onClick={() => set(next)} data-nuclide-toggle="theme" title={t("theme.title", { current: label(theme), next: label(next) })} aria-label={t("theme.aria", { current: label(theme), next: label(next) })}

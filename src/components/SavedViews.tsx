...
export function SavedViews() {
  ...
  const exportAll = () => download(`nuclide-saved-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify({ exported: new Date().toISOString(), views: loadViews(), watchlist: loadWatchlist() }, null, 2), "application/json");
    ...
    <div className="space-y-12">
      <section aria-labelledby="h-watch">
        ...
        {!ready ? <p className="text-sm text-muted">Loading…</p> : watching.length === 0 ? (
          <div className="card p-5 text-sm text-muted">
            ...
            <p className="mt-2">Try <Link className="underline" href="/drugs/pluvicto/">Pluvicto</Link>, <Link className="underline" href="/indications/prostate-mcrpc/">metastatic castration-resistant prostate cancer</Link> or <Link className="underline" href="/targets/psma/">PSMA</Link>.</p>
          </div>
        ) : (
      ...
      <section aria-labelledby="h-views">
        ...
        {!ready ? <p className="text-sm text-muted">Loading…</p> : views.length === 0 ? (
          <div className="card p-5 text-sm text-muted">
            <p>No saved views yet. Every table on Nuclide keeps its filters, search and sort in the URL; the <span className="rounded-md border border-border bg-card px-1.5 py-0.5 text-xs text-foreground">Save view</span> button next to the count stores that URL under a name. Views come back exactly as you left them.</p>

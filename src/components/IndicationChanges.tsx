...
import { ChangesGlyph } from "./MyIndication";

/**
 * The dated changes for a cancer, grouped by month, newest first. Each row links to the record it came from and
 * names the field the date was read from. Shared by the full page (/indications/<id>/changes/) and the cancer tab.
 */
...
export function ChangesGroups({ groups }: { groups: ChangeGroup[] }) {
  if (!groups.length) return <p className="text-sm text-muted">No dated changes are recorded for this record yet.</p>;
  return (
...
export function FollowLine({ indication }: { indication: { id: string; name: string; route: string; asOf: string } }) {
  return (
    <div className="card p-3 flex flex-wrap items-center gap-3 text-sm">
      ...
      <span className="text-muted">Watch this record and <Link href="/saved/" className="underline hover:text-foreground">Saved</Link> shows what changed since you last looked.</span>
      <span className="ms-auto"><WatchButton id={indication.id} kind="indication" name={indication.name} route={indication.route} asOf={indication.asOf} /></span>
    </div>
  );
}

/** A short preview for the entity page tab: the newest items and the link to the full list. */
export function ChangesPreview({ items, total, href }: { items: ChangeItem[]; total: number; href: string }) {
  return (
    <div>
      <div className="flex flex-wrap items-baseline justify-between gap-2 mb-3">
        <p className="text-sm text-muted">Dated changes read from the records linked to this indication: approvals, regulatory steps, reported trials, guideline versions and milestones. Newest first; no date is inferred.</p>

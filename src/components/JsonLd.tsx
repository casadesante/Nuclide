...
/** Wikidata item URI for a record, from the generated map (same form as the owl:sameAs triples in nuclide.nt). */
...
/** Reference to another Nuclide record: name plus the page it lives on. */
...
/** Organization reference: company/institution website when we have one, else the Nuclide page. */
  ...
  switch (e.kind) {
    ...
    case "trial":
      return {
        ...
        healthCondition: e.indications.length ? e.indications.map((id) => ref(id, "MedicalCondition")).filter(Boolean) : undefined,
    ...
    <Script
      data={{
        ...
        distribution: [
          ...
          dl("/api/v1/nuclide.nt", "application/n-triples", "RDF triples"),

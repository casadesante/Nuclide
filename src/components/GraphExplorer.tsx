...
function glyphFor(n: GraphNode, className: string) {
  if (n.kind === "indication") return <IndicationIcon indicationId={n.id} className={className} />;
...
/**
 * SVG radial explorer. Overview: fronts on the inner ring, indications on the outer. Focus: one object in the centre,
    ...
    <div className={`gx ${dim ? "is-dim" : ""}`}>
      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:items-start">
        ...
        <div className="card relative flex-1 min-w-0 overflow-hidden" style={{ aspectRatio: `${box.W} / ${box.H}` }}>
          <svg viewBox={`0 0 ${box.W} ${box.H}`} className="absolute inset-0 h-full w-full select-none" role="group" aria-label={focusNode && focus !== null ? `${focusNode.name} and its ${adj[focus].length} links` : "Nuclide knowledge graph: fronts and indications"} onMouseLeave={() => { setHover(null); setTip(null); }}>
            <defs>
              {gradientKinds.map((k) => (
            ...
            {focus === null && (
              <g>
                ...
                {compact && (
                  <g>
                    <text x={cx} y={cy - 4} textAnchor="middle" className="gx-center-label">Nuclide</text>
          ...
          {!compact && focus === null && (
            <div className="absolute z-[1] w-32 -translate-x-1/2 -translate-y-1/2 card px-2 py-2 text-center" style={{ left: "50%", top: "50%" }}>
              <p className="text-sm font-semibold">Nuclide</p>
    ...
    return (
      <div className="card p-4 space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted leading-snug">Every page on Nuclide is a node and every reference between pages is a link. Hover a front or a cancer to light up its links, or start from one of the busiest objects.</p>
        </div>
        <div>

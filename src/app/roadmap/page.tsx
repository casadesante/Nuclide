...
export const metadata: Metadata = { title: "Roadmap", description: "How Nuclide keeps expanding and stays current: corpus health gauges computed at build, every idea by status, and the method behind both." };
...
const EXPANSION: MethodCard[] = [
  {
    ...
    runs: "Enumerate the universe (isotopes in clinical or trial use, FDA/EMA approvals, indications, guideline tables), diff it against the corpus, and write the gaps down with size and owner. The gauges above are the automated half; an editorial pass at the start of each wave is the other.",
    cadence: "Gauges: every build. Editorial audit: at the start of each wave.",
    code: [{ label: "src/lib/health.ts", path: "src/lib/health.ts" }, { label: "/gaps/", path: "src/app/gaps/page.tsx" }],
    better: "Open one GitHub issue per open gap automatically, so the audit becomes a work queue rather than a document.",
  },
  {
    ...
    <>
      ...
      <Container className="pb-16">
        <nav aria-label="Sections" className="flex flex-wrap gap-x-4 gap-y-1 text-sm mb-8">
          {[["#health", "A. Corpus health"], ["#ideas", "B. Ideas by status"], ["#method", "C. Method"], ["#propose", "D. Propose an idea"]].map(([href, label]) => (

...
export const metadata: Metadata = pageMeta({ title: "Suggest an edit", description: "How anyone, including the organisations described, can correct or extend an Nuclide record: one gate, an issue form with a source, reviewed and validated before it goes live.", path: "/suggest/" });
...
const FORMS: Array<{ t: IssueTemplate; label: string; when: string; title: string }> = [
  ...
  { t: "new-object", label: "New object", when: "An isotope, agent, indication, target, trial, company, institution, person or idea is missing.", title: "add: " },
...
export default function SuggestPage() {
  return (
    <>
      <PageHeader kicker={<GroupKicker id="learn" />} title="Suggest an edit" lede="Every record has a form. Propose a change with a source, say who you are, declare any conflict, and it becomes a GitHub issue that a maintainer safety-checks, validates and turns into a merged edit. Organisations can update their own records the same way. Nothing on Nuclide is edited directly." />
      <Container className="pb-16 max-w-3xl prose-nuclide text-[15px] leading-relaxed space-y-8">
        <section>
        ...
        <section>
          ...
          <p>Contributions are published as part of the Nuclide corpus under CC BY-NC 4.0 (data, free for non-commercial use, commercial use licensed by Nuclide) and MIT (code). Submitting the form confirms you agree.</p>
        </section>
      </Container>
    </>
  );

...
const STORAGE: ReadonlyArray<{ name: string; where: string; holds: string }> = [
  ...
  { name: "nuclide:analytics", where: "Local storage", holds: "Your analytics choice: granted or denied. Absent until you choose." },
  { name: "nuclide:theme", where: "Local storage", holds: "Light, dark, high-contrast or system theme." },
  { name: "nuclide.layer", where: "Local storage", holds: "Reading level (technical, plain, simple) and site language." },
  { name: "nuclide:region", where: "Local storage", holds: "The region you chose for approvals and access." },
  { name: "nuclide:profile:v1", where: "Local storage", holds: "A reading-mode and indication preference field defined in the site's code; no page currently offers a way to set it." },
  { name: "nuclide:watchlist:v1", where: "Local storage", holds: "Pages you pressed Watch on, with the dates you last saw them." },
  { name: "nuclide:saved-views:v1", where: "Local storage", holds: "Table views you saved: a name and the address that reproduces the filters." },
  { name: "nuclide:shortcuts:v1", where: "Local storage", holds: "Whether keyboard shortcuts are on or off." },
  { name: "nuclide:missed-paths", where: "Local storage", holds: "The last fifty addresses this browser asked for that did not exist, so the not-found page can suggest where you meant to go." },
  { name: "nuclide:stars", where: "Local storage", holds: "The GitHub star count shown in the header, cached for an hour." },
  { name: "nuclide.translate-offer.dismissed", where: "Local storage", holds: "That you dismissed the offer to switch language." },
  { name: "nuclide-v3-pages, -shell, -assets, -data, -media", where: "Cache storage (service worker)", holds: "Copies of pages you have visited (up to 200), the site's scripts and styles, its data files and images, so the site works offline." },
];
...
export default function PrivacyPage() {
  ...
  return (
    <>
      <WebPageJsonLd path="/privacy/" name={`${TITLE} · Nuclide`} description={DESCRIPTION} dateModified={date.iso} />
      <PageHeader
        ...
        title={TITLE}
        lede="What Nuclide collects, where it lives and what leaves your device and where it goes, written from the code that runs the site. The site is open source, so every statement here can be checked against the file it describes."
      ...
      />
      <Container className="pb-16 prose-nuclide text-[15px] leading-relaxed max-w-3xl space-y-10">
        ...
        <LegalSection {...S["on-your-device"]}>
          <p><strong>Reading mode and region.</strong> The <Term tip="A small store inside your browser that a website can write to. Only that website, on that device, can read it.">local storage</Term> keys <code>nuclide.layer</code> and <code>nuclide:region</code> hold your reading level, site language and the region you chose for approvals and access, on this device only. They are never sent to Nuclide or anyone else and do not follow you to another browser or device.</p>
          <p><strong>Saved items.</strong> The pages you press Watch on and the table views you save are browser-only in the same way. The <Link href="/saved/">Saved</Link> page checks for changes by fetching Nuclide&apos;s own static data files for the pages on your list; export and import move the lists between browsers as a file you keep.</p>
          <p><strong>Clearing.</strong> Remove watched pages and saved views on <Link href="/saved/">Saved</Link>, or use your browser&apos;s site data controls for nuclide.cc to remove everything in the table below from the device.</p>
        </LegalSection>
        ...
        <LegalSection {...S.storage}>
          ...
          <p>To clear any of it, use your browser&apos;s site data or cookie controls for nuclide.cc.</p>
        </LegalSection>
        ...
        <LegalSection {...S["third-parties"]}>
          ...
          <p>Each of these sees your IP address and the request, as any website you visit does, and each has its own privacy policy. Links that leave nuclide.cc open in a new tab; those sites&apos; policies apply once you are there.</p>
        </LegalSection>

        <LegalSection {...S["no-ads"]}>
          <p>Nuclide shows no advertising, loads no advertising trackers and does not sell, rent or share personal data for marketing or any other purpose. The only measurement is Google Analytics as described above. Nothing about you is used to train models or shared with data brokers.</p>
        </LegalSection>
        ...
        <LegalSection {...S.children}>
          <p>Nuclide is not directed at children under 16 and we do not knowingly collect personal data from them. If you believe we hold any, <a href="#contact">contact us</a> and we will have it removed.</p>
        </LegalSection>

...
export default function TimelinePage() {
  ...
  const data: TimelineData = {
    ...
    trials: g.kind("trial").filter((t) => t.yearReported).map((t) => ({ year: t.yearReported!, name: t.name, route: routeFor(t), status: t.status, indications: t.indications.map((id) => short(g.must(id).name)) })).sort((a, b) => a.year - b.year),
    history: g.kind("indication").flatMap((c) => c.history.filter((h) => typeof h.year === "number" || /^\d{4}$/.test(String(h.year))).map((h) => ({ year: Number(h.year), indication: short(c.name), cancerRoute: routeFor(c), title: h.title, note: h.note, refs: h.refs.map((id) => ({ name: g.must(id).name, route: routeFor(g.must(id)) })) }))).sort((a, b) => a.year - b.year),

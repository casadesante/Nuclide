...
export const metadata: Metadata = pageMeta({ title: "Open evaluation", description: "A public benchmark of questions a patient or clinician might ask, scored against Nuclide and any other system with the same rubric.", path: "/eval/" });
...
function loadRuns(): { runs: RunSummary[]; latestOnco?: { summary: RunSummary; results: RunResult[] } } {
  ...
  const oncoFile = files.filter((f) => f.startsWith("nuclide-")).pop();
  ...
  return (
    <>
      ...
      <Container className="pb-16">
        ...
        <p className="text-xs text-muted mt-2 max-w-3xl">Nuclide&apos;s own run is a deliberately hard retrieval proxy: it scores only the TL;DR and summary text of the top eight search hits, with no reasoning layer. A human reading the linked pages would score higher; that gap is part of what the benchmark measures.</p>
        ...
        <div className="card overflow-x-auto">
          <table className="nuclide">
            <thead><tr><th>#</th><th>Question</th><th>Category</th><th>Level</th><th>Grounded in</th><th>Nuclide</th></tr></thead>
            <tbody>
              {benchmark.map((q) => {

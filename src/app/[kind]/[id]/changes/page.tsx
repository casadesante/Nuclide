...
import { ChangeKindChip, ChangesGroups, FollowLine } from "@/components/IndicationChanges";

const CANCERS = KIND_META.indication.route;

/** Only indications have a changes page; the segment lives under [kind]/[id] so the URL is /indications/<id>/changes/. */
export function generateStaticParams() {
  return graph().kind("indication").map((c) => ({ kind: CANCERS, id: c.id }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ kind: string; id: string }> }): Promise<Metadata> {
  ...
  if (kind !== CANCERS || !c || c.kind !== "indication") return {};
...
export default async function EntityChangesPage({ params }: { params: Promise<{ kind: string; id: string }> }) {
  const { kind, id } = await params;
  const g = graph();
  const c = g.get(id);
  if (kind !== CANCERS || !c || c.kind !== "indication") notFound();
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        logo={<span className="inline-flex h-16 w-16 items-center justify-center rounded-2xl border border-accent/30 bg-accent-soft text-accent"><IndicationIcon indicationId={c.id} className="h-10 w-10" /></span>}
        right={<Link href={route} className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium">Indication page →</Link>}
      />
      <Container className="pb-16">
        ...
        <div className="mb-8"><FollowLine indication={{ id: c.id, name: c.name, route, asOf: c.asOf }} /></div>

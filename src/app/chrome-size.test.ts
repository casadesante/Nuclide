...
import { myCancerList, myIndicationTiles } from "@/lib/my-indication-list";
 ...
 *
 *  - the layout never calls the graph for the cancer list (it is fetched on demand, src/lib/use-my-indication-list.ts);
...
vi.mock("@/lib/my-indication-list", async (importOriginal) => {
  const mod = await importOriginal<typeof import("@/lib/my-indication-list")>();
  return { ...mod, myCancerList: vi.fn(mod.myCancerList), myIndicationTiles: vi.fn(mod.myIndicationTiles) };
});
...
describe("shared chrome stays small", () => {
  ...
  it("does not serialise the cancer list into the layout", () => {
    ...
    expect(vi.mocked(myIndicationTiles)).not.toHaveBeenCalled();
    // The header's My cancer chip and account menu render their unset state; no cancer routes beyond the nav's own links.
    expect((layoutHtml.match(/href="\/indications\//g) ?? []).length).toBeLessThanOrEqual(4);

...
export default function PivotPage() {
  ...
  const empty = (): Record<Dim, string[]> => ({ indication: [], target: [], modality: [], company: [], front: [], status: [], phase: [] });

  const facts: Fact[] = [];
  for (const d of g.kind("drug")) {
    const dims = empty();
    dims.indication = names(d.indications); dims.target = names(d.targets); dims.modality = [modalityClass(d.modality)]; dims.company = names(d.companies); dims.front = frontsOf(d.sections, d.technologies); dims.status = d.status ? [d.status] : []; dims.phase = d.status?.startsWith("phase") ? [d.status.replace("phase-", "Phase ")] : d.status === "approved" ? ["Approved"] : [];
  ...
  for (const t of g.kind("trial")) {
    ...
    dims.indication = names(t.indications); dims.target = names([...new Set([...t.targets, ...drugs.flatMap((d) => d.targets)])]); dims.modality = [...new Set(drugs.map((d) => (d.kind === "drug" ? modalityClass(d.modality) : "")).filter(Boolean))]; dims.company = names([...new Set([...t.companies, ...drugs.flatMap((d) => d.companies)])]); dims.front = frontsOf(t.sections, [...t.technologies, ...drugs.flatMap((d) => d.technologies)]); dims.status = t.status ? [t.status] : []; dims.phase = [phaseLabel(t.phase)];
    facts.push({ id: t.id, kind: "trial", name: t.name, route: routeFor(t), status: t.status, rank: 0, dims });
  }
  for (const x of g.kind("technology")) {
    const dims = empty();
    dims.indication = names(x.indications); dims.target = names(x.targets); dims.modality = [x.generation ?? "-"]; dims.company = names([...new Set([...x.companies, ...(g.incoming(x.id).get("company") ?? []).map((c) => c.id)])]); dims.front = x.sections.map((id) => g.must(id).name); dims.status = x.status ? [x.status] : []; dims.phase = x.status?.startsWith("phase") ? [x.status.replace("phase-", "Phase ")] : x.status === "approved" || x.status === "standard-of-care" ? ["Approved / standard"] : [];
    facts.push({ id: x.id, kind: "technology", name: x.name, route: routeFor(x), status: x.status, rank: 0, dims });
  }

  const meta: DimMeta = {
    indication: { label: "Indication", ids: Object.fromEntries(g.kind("indication").map((c) => [short(c.name), c.id])), routes: Object.fromEntries(g.kind("indication").map((c) => [short(c.name), routeFor(c)])) },
    target: { label: "Target", routes: Object.fromEntries(g.kind("target").map((t) => [short(t.name), routeFor(t)])) },
    modality: { label: "Modality", routes: {
      "Radioligand therapy": routeFor(g.must("radioligand-therapy")), "Targeted alpha therapy": routeFor(g.must("targeted-alpha-therapy")),
      "Peptide receptor radionuclide therapy": routeFor(g.must("prrt")), "Radioimmunotherapy": routeFor(g.must("radioimmunotherapy")),
      "PET tracer": routeFor(g.must("pet")), "SPECT tracer": routeFor(g.must("spect")), "Radioiodine": routeFor(g.must("radioiodine-therapy")),
      "Radioembolisation": routeFor(g.must("radioembolisation-tare")), "Brachytherapy": routeFor(g.must("ldr-seed-brachytherapy")),
      "Auger therapy": routeFor(g.must("auger-electron-therapy")), "Dosimetry": routeFor(g.must("radioligand-dosimetry")),
    } },

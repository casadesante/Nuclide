...
const DESCRIPTION = "The rules for using Nuclide in plain words: an information resource, not medical advice; accuracy not guaranteed; accounts, licences, external links, liability, changes and how to contact us.";
...
const SECTIONS: readonly LegalSectionDef[] = [
  { id: "purpose", title: "What Nuclide is for", icon: "compass" },
...
export default function TermsOfUsePage() {
  ...
  return (
    <>
      <WebPageJsonLd path="/terms-of-use/" name={`${TITLE} · Nuclide`} description={DESCRIPTION} dateModified={date.iso} />
      <PageHeader
        ...
        title={TITLE}
        lede="The rules for using Nuclide, written in plain words. They apply to nuclide.cc and to everything it publishes: the website, the API, the feeds, the downloads and the text files for language models. By using them you agree to these terms; if you do not agree, please do not use the site."
      ...
      <Container className="pb-16 prose-nuclide text-[15px] leading-relaxed max-w-3xl space-y-10">
        ...
        <LegalSection {...S.purpose}>
          <p>Nuclide is a public, cited map of radiopharmaceuticals: isotopes, agents, indications, targets, trials, companies, institutions, people, pathways and ideas, one page each, built in the open and linked to primary sources. It exists so that a patient, a carer, a clinician, a researcher, an investor or a policymaker can see the current state of the art, the history and what is coming for any isotope, agent or indication, and follow the links to check it for themselves.</p>
          <p>The site is run by Nuclide (<Placeholder>[legal entity and address]</Placeholder>). The code and the data are published at <a href={REPO_URL} rel="noopener">{REPO_URL.replace("https://", "")}</a>, so you can read how every page is made. How the site is built and its rules for facts are on the <Link href="/about/">About page</Link>.</p>
        </LegalSection>

        <LegalSection {...S["not-medical-advice"]}>
          <p><strong>Nuclide is an information resource. It is not medical advice, diagnosis or treatment, and it does not know your case.</strong> Nothing here replaces a conversation with a qualified clinician who knows your history, your test results and your wishes. Using the site does not create a doctor and patient relationship, or any other professional relationship, between you and Nuclide or anyone who contributes to it.</p>
          ...
          <p>Dose figures, calculators and trial-finder results on this site are reference material for a discussion with your care team. They are not instructions, and they cannot take account of your kidneys, your liver, your other medicines or anything else about you.</p>
        </LegalSection>

        <LegalSection {...S.accuracy}>
          <p>Content is built and checked in the open, mostly from public sources, and much of it has not yet been reviewed by a named expert; pages that have been reviewed carry a badge naming the reviewer and the date. Facts may be incomplete, out of date or wrong. The field changes weekly: trial results, approvals, labels and guidelines move faster than any single page.</p>
        ...
        <LegalSection {...S["your-responsibilities"]}>
          <p>When you use Nuclide you agree to:</p>
          <ul className="list-disc pl-5 space-y-1.5">
            ...
            <li>not present Nuclide content as medical advice, or as endorsed by Nuclide, and keep the attribution when you reuse the data;</li>
        ...
        <LegalSection {...S.licences}>
          <p><strong>Content.</strong> The Nuclide corpus (the text, structure, curation and derived files under <code>src/data/</code>) is copyright Nuclide and its contributors and is licensed under <a href="https://creativecommons.org/licenses/by-nc/4.0/" rel="noopener">Creative Commons Attribution-NonCommercial 4.0</a> (<Term tip="You may copy, share and adapt the material for non-commercial purposes as long as you credit the source. Commercial use needs a separate agreement.">CC BY-NC 4.0</Term>). It is free for individual, educational, academic, charitable and other non-profit use with the attribution “Data from Nuclide (nuclide.cc)” and a link to <Link href="/">nuclide.cc</Link>. Commercial use, for example inside a paid product or service, advertising-funded redistribution or a company data pipeline, must contact us to pay for the data: ask through a <a href={`${REPO_URL}/issues/new?labels=licensing&title=Commercial+licence+request`} rel="noopener">licensing request</a>. The full terms are in <a href={`${REPO_URL}/blob/main/LICENSE-DATA`} rel="noopener">LICENSE-DATA</a>.</p>

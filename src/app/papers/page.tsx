...
export const metadata: Metadata = pageMeta({ title: "Publishing trends", description: "Fastest-growing topics in the cancer literature, computed weekly from Europe PMC for every product, target, cancer, and technology in Nuclide.", path: "/papers/" });
...
const KINDS: Array<{ kind: string; label: string; minPrior: number }> = [
  ...
  { kind: "indication", label: "Indications", minPrior: 100 },
];
  ...
  return (
    <>
      <PageHeader kicker={<GroupKicker id="intel" />} title="Publishing trends"
        lede="Every object in Nuclide has a literature query. Once a week we ask Europe PMC how many papers matched it in each year and in the last twelve months, then rank topics by how fast they are growing. This is the pulse of research effort, not of results: a surge in papers is a surge in attention." />
        ...
        {index && (
          <>
            <div className="text-sm text-muted flex flex-wrap gap-x-4 gap-y-1">
              ...
              <span>Refreshed weekly by <a className="underline" href="https://github.com/casadesante/Nuclide/blob/main/.github/workflows/refresh-papers.yml" rel="noopener">GitHub Actions</a></span>
            </div>

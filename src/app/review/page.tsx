...
export const metadata: Metadata = pageMeta({ title: "Review: model panel and human queue", description: "A panel of named AI models comments on Nuclide records (machine commentary, not clinical review) and a queue ranks which pages most need a named clinical, scientific, regulatory or patient-advocate reviewer.", path: "/review/" });
    ...
    <>
      ...
      <Container className="pb-16">
        ...
        <section id="coverage" className="scroll-mt-24 mt-12">
          ...
          <p className="text-sm text-muted mb-3 max-w-3xl">Named clinicians, scientists, regulatory specialists and patient advocates sign pages off on top of the model panel. A human review is a statement about a page on a date and is the only review Nuclide treats as clinical.</p>
          <div className="grid gap-3 sm:grid-cols-3 mb-4">
              ...
              return (
                <div key={t} className="card p-4">
                  ...
                  <ol className="list-decimal pl-5 space-y-1 text-sm">
                    {rows.map((q) => (
                      <li key={q.id} className="flex flex-wrap items-baseline justify-between gap-x-3"><Link href={q.route} className="hover:underline">{q.name}</Link><a className="text-xs underline text-muted" href={issueUrl("review", { entity: `${q.id} (${q.kind}) · ${q.name}`, track: t, page: `https://nuclide.cc${q.route}` }, { title: `review: ${q.id}` })} rel="noopener">review</a></li>
                    ))}

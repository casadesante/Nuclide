...
export const metadata: Metadata = pageMeta({ title: "Build on Nuclide", description: "Recipes, an OpenAPI spec, TypeScript types, embeddable cards and the MCP server: everything a developer needs to build a trial matcher, a dashboard or a chatbot on the Nuclide corpus.", path: "/build/" });
...
export default function BuildPage() {
  ...
  return (
    <>
      <PageHeader kicker={<GroupKicker id="learn" />} title="Build on Nuclide"
      ...
      <Container className="pb-16 max-w-4xl">
        <div className="card p-4 text-sm border-accent/30 bg-accent-soft/40">
          ...
          <ul className="list-disc pl-5 space-y-1">
            ...
            <li><a className="underline" href="https://github.com/casadesante/Nuclide/blob/main/src/lib/schema.ts" rel="noopener">src/lib/schema.ts</a>: the Zod schema that every object is validated against at build time. It is the source of truth for field names and enums.</li>
            <li><a className="underline" href="https://github.com/casadesante/Nuclide/tree/main/mcp" rel="noopener">MCP server</a>: <code className="text-xs">npm run mcp</code> exposes search, get_entity, list_kind, for_cancer and rank tools over stdio.</li>
          </ul>
        </div>
        ...
        <Section title="Types in your project">
          ...
          <Code>{`// types/nuclide.ts: copy src/lib/schema.ts from the repo next to this file, then
...
export async function getEntity(id: string): Promise<EntityResponse> {
  ...
  if (!r.ok) throw new Error(\`Nuclide: \${id} not found\`);
...
const indicationId = "tnbc";
const biomarkers = ["trop2", "pdl1", "brca"]; // Nuclide target ids; see /targets/

const { neighbours } = await fetch(\`\${SITE}/api/v1/entities/\${indicationId}.json\`).then((r) => r.json());
...
// Everything that links to HER2, grouped by kind (trials, indications, pathways, papers...)
...
const { built, counts } = await fetch("${SITE}/api/v1/meta.json").then((r) => r.json());`}</Code>
        ...
        <Section title="Recipe 3: a chatbot over MCP">
          ...
          <Code>{`git clone https://github.com/casadesante/Nuclide.git && cd Nuclide && npm ci

# Claude Desktop, Cursor, or any MCP client: add a stdio server
{
  "mcpServers": {
    "nuclide": { "command": "npm", "args": ["run", "mcp"], "cwd": "/path/to/Nuclide" }
  }
}

# Tools exposed: search(query, limit), get_entity(id), list_kind(kind),
# for_cancer(indicationId), rank(...). Every answer carries the object url so the
# assistant can cite nuclide.cc and the reader can check the source.`}</Code>
        ...
        <Section title="Embeddable cards">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {examples.map((e) => e && (
              <div key={e.id}>
                <iframe src={`/embed/${e.id}/`} title={`${e.name} on Nuclide`} width="100%" height="190" style={{ border: 0, borderRadius: 12 }} loading="lazy" />
          ...
          <div className="card overflow-x-auto">
            <table className="nuclide">
              ...
              <tbody>
                ...
                <tr><td className="font-mono text-xs">/api/v1/my-indications.json</td><td>Every cancer as id, name, route and hub group (the chooser list)</td></tr>
        ...
        <Section title="Licence and attribution">
          <div className="text-[15px] leading-relaxed space-y-2">
            <p>The data are <a className="underline" href="https://creativecommons.org/licenses/by-nc/4.0/" rel="noopener">CC BY-NC 4.0</a> (free for non-commercial use; commercial use needs a licence from Nuclide) and the code is MIT. Name Nuclide and link to <a className="underline" href={SITE}>nuclide.cc</a> wherever the data or text derived from it appears, for example <code className="text-xs">Data from Nuclide (nuclide.cc), CC BY 4.0</code>. The same notice is in <code className="text-xs">/api/v1/meta.json</code> so it can travel with the data.</p>
            <p>Logos remain their owners&rsquo; trademarks; molecule structures keep their PubChem and RCSB terms; ClinicalTrials.gov data are public domain. Nothing here is medical advice, and every record carries the date it was last checked: show it.</p>

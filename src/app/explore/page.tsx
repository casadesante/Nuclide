...
export default function Explore() {
  const g = graph();
  const indications: PowerCancer[] = g.kind("indication").map((c) => ({ id: c.id, name: c.name, group: c.group, route: routeFor(c) }));
  ...
  return (
    <>
      ...
      <Container className="pb-16">
        <Suspense><PowerView rows={rows} indications={indications} /></Suspense>
      </Container>
    </>
  );
}

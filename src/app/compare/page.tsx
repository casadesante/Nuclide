...
export const metadata: Metadata = pageMeta({ title: "Compare", description: "Side-by-side comparison of up to five products, technologies, targets, trials, or indications, with differences highlighted.", path: "/compare/" });
...
export default function ComparePage() {
  ...
  const items: CompareItem[] = [
    ...
    ...g.kind("target").map((t): CompareItem => ({
      ...
      fields: [
        ...
        ["Prevalence", t.prevalence.length ? t.prevalence.map((p) => `${g.must(p.indicationId).name.replace(/ \(.*\)$/, "")}: ${p.pct}%`).join(" · ") : "-"],
        ["Cancers", names(t.indications)],
    ...
    ...g.kind("indication").map((c): CompareItem => ({
      id: c.id, kind: "indication", name: c.name, route: routeFor(c), status: c.status, tldr: c.tldr,
      fields: [
        ["Group", c.group],
        ["Burden", c.burden ?? "-"],
  ...
  return (
    <>
      <PageHeader kicker={<GroupKicker id="find" />} title="Compare" lede="Up to five products, technologies, targets, trials, or indications side by side, same fields, differences highlighted. The URL is shareable." />

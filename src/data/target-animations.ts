...
function her2Scene(): Mesh {
  ...
  return frame(sc, 14, (t, pts, alpha) => {
    hide(alpha, dmg, bys);
    let caption = "";
    if (t < 0.22) { recs.forEach((p) => setAlpha(alpha, p, pulse(t, 4))); caption = "1 · HER2 crowds the surface of some breast, stomach and other indications (millions of copies per cell)"; }
...
function nuclearReceptorScene(o: NrOpts): Mesh {
  ...
  return frame(sc, 12, (t, pts, alpha) => {
    ...
    const labels: Lbl[] = [L([C[0], C[1] + r + 0.1, 0], "Indication cell"), L([N[0], N[1] - 0.75, 0], "Nucleus (DNA)")];
...
function suppressorScene(): Mesh {
  ...
  return frame(sc, 12, (t, pts, alpha) => {
    ...
    else if (t < 0.5) { const u = phase(t, 0.25, 0.5); setAlpha(alpha, dmg, 1); setAlpha(alpha, sig, 1 - u); moveTo(pts, base, brake, B0, B1, 1 - u); setAlpha(alpha, brake, 1 - 0.8 * u); spin(0); caption = "2 · In most indications the brake gene (TP53, BRCA, RB) is lost or broken"; }
...
function syntheticLethalityScene(): Mesh {
  ...
  return frame(sc, 13, (t, pts, alpha) => {
    ...
    const labels: Lbl[] = [L([C[0], C[1] + r + 0.1, 0], "Indication cell"), L([N[0], N[1] - 1.1, 0], "DNA")];
...
function genericScene(): Mesh {
  ...
  return frame(sc, 11, (t, pts, alpha) => {
    ...
    const labels: Lbl[] = [L([C[0], C[1] + r + 0.1, 0], "Indication cell"), L([N[0], N[1] - 0.45, 0], "Inside the cell"), L([X0 - 0.35, 0.3, 0.1], "Surface target"), L([INNER[0], INNER[1] - 0.3, INNER[2]], "Inside target")];
...
function bcl2Scene(): Mesh {
  ...
  return frame(sc, 12, (t, pts, alpha) => {
    ...
    const labels: Lbl[] = [L([C[0], C[1] + r + 0.1, 0], "Indication cell"), L([MITO[0], MITO[1] - 0.6, 0.1], "Mitochondrion (power plant)"), L([B[0] + 0.55, B[1] + (t >= 0.75 ? 0.35 * phase(t, 0.75, 1) : 0), B[2]], "BCL-2 (the hand over the trigger)"), L([B[0] - 0.45, B[1] - 0.3 - (t >= 0.75 ? 0.25 * phase(t, 0.75, 1) : 0), B[2]], "BAX / BAK (self-destruct triggers)")];
...
export const TARGET_ANIMATED: Record<string, () => Mesh> = {
  ...
  egfr: () => rtkScene({ receptor: "EGFR receptor", ligand: "Growth signal (EGF)", drug: "TKI (osimertinib) plugs the ATP pocket", mutant: "point", captions: [
    "1 · EGFR normally waits for a growth signal; in some lung indications a mutation keeps it switched on with no signal",

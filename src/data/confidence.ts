 ...
 * steps: that the step is reached). These are initial estimates by the Nuclide editors on
...
const ed = (lo: number, hi: number, note?: string): Confidence => ({ probability: [lo, hi], by: "Nuclide editors (initial estimate)", on: "2026-09-07", note });
...
export const confidence: Record<string, Confidence> = {
  ...
  "idea-exercise-as-adjuvant": ed(0.5, 0.75, "CHALLENGE gives level-1 evidence in colon cancer; replication in other indications is likely but slow."),

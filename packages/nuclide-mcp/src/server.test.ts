...
import { ATTRIBUTION, NuclideClient } from "../../nuclide-cli/src/client";
import { writeFixture } from "../../nuclide-cli/src/test-fixture";
import { compareRecords, createServer } from "./server";

let cleanup = async () => {};
let mcp: Client;
let api: NuclideClient;

beforeAll(async () => {
  const f = await writeFixture();
  cleanup = f.cleanup;
  api = new NuclideClient({ base: f.dir, local: true });
...
describe("nuclide-mcp", () => {
  it("exposes the six tools, the kind resources and the brief prompt", async () => {
    ...
    expect(resources).toContain("nuclide://kinds");
    const templates = (await mcp.listResourceTemplates()).resourceTemplates.map((t) => t.uriTemplate);
    expect(templates).toContain("nuclide://kinds/{kind}");
    expect((await mcp.listPrompts()).prompts.map((p) => p.name)).toEqual(["nuclide-brief"]);
  });
  ...
  it("every tool result carries the attribution", async () => {
    ...
    const entity = json(await mcp.callTool({ name: "get_entity", arguments: { id: "/indications/tnbc/" } }));
    expect((entity.entity as { id: string }).id).toBe("tnbc");
    expect(entity.url).toBe("https://nuclide.cc/indications/tnbc/");

...
const sg = {
  ...
  approvals: [{ region: "US", year: 2020, indication: "mTNBC after two prior therapies" }], targets: ["trop2"], indications: ["tnbc"], trials: ["ascent"], companies: [], related: [],
...
const dato = {
  ...
  approvals: [{ region: "US", year: 2025, indication: "HR+/HER2- breast cancer" }], targets: ["trop2"], indications: ["tnbc"], trials: [], companies: [], related: [],
  toxicity: [{ event: "Stomatitis", grade3PlusPct: 7 }],
};
const tnbc = {
  id: "tnbc", kind: "indication" as Kind, name: "Triple-negative breast cancer (TNBC)", aka: [], tldr: "Breast cancer with no ER, PR or HER2; about 15% of cases and the hardest subtype to treat.",
  summary: "Triple-negative breast cancer lacks the three receptors that other breast indications are treated through. Chemotherapy, immunotherapy and antibody-drug conjugates are the main options.",
...
export async function writeFixture(): Promise<{ dir: string; cleanup: () => Promise<void> }> {
  const dir = await mkdtemp(join(tmpdir(), "nuclide-api-"));
  ...
  await w("meta.json", { built: "2026-09-10T00:00:00.000Z", version: "test", counts: { drug: 2, indication: 1 }, total: 3, license: "test" });
  ...
  await w("indications.json", [tnbc]);
  await w("drugs.csv", "# Data from Nuclide, test fixture\nid,name,status\nsacituzumab-govitecan,Sacituzumab govitecan,approved\ndatopotamab-deruxtecan,Datopotamab deruxtecan,approved\n");
  await w("entities/sacituzumab-govitecan.json", { entity: sg, route: routeFor(sg), neighbours: { indication: [nb(tnbc)], drug: [nb(dato)] } });
  await w("entities/datopotamab-deruxtecan.json", { entity: dato, route: routeFor(dato), neighbours: { indication: [nb(tnbc)], drug: [nb(sg)] } });

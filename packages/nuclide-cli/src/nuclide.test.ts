...
async function cli(...argv: string[]) {
  ...
  const code = await run(argv, { out: (s) => out.push(s), err: (s) => err.push(s) }, { NUCLIDE_API: api });
  return { code, out: out.join("\n"), err: err.join("\n") };
}

describe("nuclide CLI against a local API copy", () => {
  ...
  it("get prints the record and ends with the attribution line", async () => {
    ...
    expect(r.out).toContain("Indications: Triple-negative breast cancer (TNBC) (tnbc)");
  ...
  it("--json puts the attribution on stderr and valid JSON on stdout", async () => {
    ...
    expect(data.url).toBe("https://nuclide.cc/indications/tnbc/");
  ...
  it("ask answers from the fixture records with citations", async () => {
    ...
    expect(data.sources[0].url).toMatch(/^https:\/\/nuclide\.cc\//);

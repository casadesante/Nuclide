...
const rec: EntityRecord = {
  ...
  route: "/drugs/sg/",
  neighbours: { indication: [{ id: "tnbc", kind: "indication", name: "TNBC", route: "/indications/tnbc/" }], target: [] },
};
...
describe("formatEntity", () => {
  it("prints name, TL;DR, summary, fields, connected records and sources", () => {
    ...
    expect(out).toContain("https://nuclide.cc/drugs/sg/");
    expect(out).toContain("TL;DR\n  A TROP2 ADC.");
    expect(out).toContain("Summary\n  Approved for TNBC.");
    expect(out).toContain("Modality: ADC");
    expect(out).toContain("Indications: TNBC (tnbc)");
...
describe("formatTable and formatSearch", () => {
  ...
  it("says why each hit matched", () => {
    const out = formatSearch([{ id: "sg", kind: "drug", name: "SG", tldr: "t", route: "/drugs/sg/", url: "https://nuclide.cc/drugs/sg/", matched: { words: true, concepts: ["trop2"] } }]);

/**
 * Read-only client for the Nuclide static API (https://nuclide.cc/api/v1/ or a local copy such as out/api/v1).
 *
 * Shared by the `nuclide` CLI and the `nuclide-mcp` server. It only uses files that scripts/build-api.ts,
 ...
 *   embeddings.json / .bin    concept-search index                   ask-index.json       Ask Nuclide name index
 *   context/<id>.md           Markdown context per record
 *
 * Ask Nuclide runs the same browser-safe pipeline as the site (src/lib/ask-pipeline.ts) with the record loader
 * pointed at the API, so answers match what https://nuclide.cc/ask/ shows. The site modules are bundled in at
...
export const SITE = "https://nuclide.cc";
export const DEFAULT_API = "https://nuclide.cc/api/v1";
/** Printed once at the end of every CLI output and included in every MCP response. */
export const ATTRIBUTION = "Data from Nuclide (nuclide.cc), CC BY-NC 4.0; free for individual and educational use, commercial use must contact Nuclide to pay";
...
export class NuclideError extends Error {
  constructor(message: string, readonly code: "not-found" | "network" | "usage" = "network") { super(message); }
}

/** `--api` flag, then NUCLIDE_API, then https://nuclide.cc/api/v1. Anything that is not http(s) is read from disk. */
export type Env = Record<string, string | undefined>;

export function resolveApi(flag?: string, env: Env = process.env): ApiSource {
  const raw = (flag ?? env.NUCLIDE_API ?? DEFAULT_API).trim().replace(/\/+$/, "");
...
export class NuclideClient {
  ...
  async text(path: string): Promise<string> {
    if (this.source.local) {
      try { return await readFile(join(this.source.base, path), "utf8"); }
        ...
        if (code === "ENOENT") throw new NuclideError(`Not found: ${path} (under ${this.source.base})`, "not-found");
        throw new NuclideError(`Cannot read ${path}: ${(err as Error).message}`);
      }
    }
    const url = `${this.source.base}/${path.split("/").map(encodeURIComponent).join("/")}`;
    let res: Response;
    try { res = await fetch(url, { headers: { "user-agent": "nuclide-cli (+https://nuclide.cc/api/)" } }); }
    catch (err) { throw new NuclideError(`Cannot reach ${url}: ${(err as Error).message}`); }
    if (res.status === 404) throw new NuclideError(`Not found: ${url}`, "not-found");
    if (!res.ok) throw new NuclideError(`${url} returned ${res.status}`);
    return res.text();
  }

  async bytes(path: string): Promise<ArrayBuffer> {
    if (this.source.local) {
      const buf = await readFile(join(this.source.base, path)).catch((err: NodeJS.ErrnoException) => { throw new NuclideError(err.code === "ENOENT" ? `Not found: ${path}` : err.message, err.code === "ENOENT" ? "not-found" : "network"); });
      return buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength) as ArrayBuffer;
    }
    const url = `${this.source.base}/${path}`;
    const res = await fetch(url).catch((err: Error) => { throw new NuclideError(`Cannot reach ${url}: ${err.message}`); });
    if (res.status === 404) throw new NuclideError(`Not found: ${url}`, "not-found");
    if (!res.ok) throw new NuclideError(`${url} returned ${res.status}`);
  ...
  async json<T>(path: string): Promise<T> {
    ...
    try { return JSON.parse(raw) as T; } catch { throw new NuclideError(`${path} is not valid JSON`); }
  }
  ...
  async entity(idOrRoute: string): Promise<EntityRecord> {
    ...
    if (!id) throw new NuclideError("An id is required", "usage");
    try { return await this.once(`entity:${id}`, () => this.json<EntityRecord>(`entities/${id}.json`)); }
    catch (err) { if (err instanceof NuclideError && err.code === "not-found") throw new NuclideError(`No record with id "${id}". Try: nuclide search ${JSON.stringify(id)}`, "not-found"); throw err; }
  }
  ...
  context(idOrRoute: string): Promise<string> {
    const id = parseId(idOrRoute);
    return this.text(`context/${id}.md`).catch((err: NuclideError) => { throw err.code === "not-found" ? new NuclideError(`No context file for "${id}"`, "not-found") : err; });
  }
  ...
  private semanticIndex(): Promise<SemanticIndex | null> {
    return this.once("semantic", async () => {
      ...
      } catch (err) { if (err instanceof NuclideError && err.code === "not-found") return null; throw err; }
    });
  }

  askIndex(): Promise<AskIndex> { return this.once("ask-index", async () => decodeAskIndex(await this.json<AskIndexWire>("ask-index.json"))); }

  /** Word and concept search fused by reciprocal rank, the same retrieval the site and Ask Nuclide use. */
  async search(query: string, opts: { kind?: Kind; limit?: number } = {}): Promise<SearchHit[]> {
    const limit = opts.limit ?? 10;
    const q = query.trim();
    if (!q) throw new NuclideError("A query is required", "usage");
  ...
  /** Ask Nuclide: intent, named records, templated cited answer. Identical logic to the site's /ask/ page. */
  async ask(question: string, opts: { region?: Region; pin?: string; onStep?: (s: string) => void } = {}): Promise<AskResult> {
    const q = question.trim();
    if (q.length < 3) throw new NuclideError("A question is required", "usage");
...
export async function kindsTable(client: NuclideClient): Promise<Array<{ kind: Kind; plural: string; label: string; route: string; count: number | null; blurb: string }>> {
...
export function applyFilters<T extends Record<string, unknown>>(rows: T[], filters: string[]): T[] {
  const parsed = filters.map((f) => { const i = f.indexOf("="); if (i < 0) throw new NuclideError(`Filter "${f}" must look like key=value`, "usage"); return { key: f.slice(0, i).trim(), value: f.slice(i + 1).trim().toLowerCase() }; });

...
describe("parseArgs", () => {
  it("separates command, positionals and flags in both --key value and --key=value forms", () => {
    const p = parseArgs(["search", "PSMA", "radioligand", "--kind", "drug", "--limit=5", "--json"]);
    expect(p.command).toBe("search");
    expect(p.positionals).toEqual(["PSMA", "radioligand"]);
...
describe("resolveApi", () => {
  it("defaults to nuclide.cc and honours NUCLIDE_API", () => {
    expect(resolveApi(undefined, {})).toEqual({ base: "https://nuclide.cc/api/v1", local: false });
    expect(resolveApi(undefined, { NUCLIDE_API: "https://example.org/api/v1/" })).toEqual({ base: "https://example.org/api/v1", local: false });
  });
  it("treats a directory as a local copy and the flag beats the environment", () => {
    const r = resolveApi("out/api/v1", { NUCLIDE_API: "https://example.org" });
...
describe("parseKind and parseId", () => {
  ...
  it("reduces routes, URLs and file names to the id", () => {
    expect(parseId("prostate-mcrpc")).toBe("prostate-mcrpc");
    expect(parseId("/drugs/pluvicto/")).toBe("pluvicto");
    expect(parseId("https://nuclide.cc/indications/PROSTATE-MCRPC/")).toBe("prostate-mcrpc");
    expect(parseId("/api/v1/context/psma.md")).toBe("psma");
  });
});

...
function contextMarkdown(e: Entity): string {
  const parts = [`# ${e.name}`, "", `Source: ${url(e)}`, `Nuclide record \`${e.id}\` (${KIND_META[e.kind].label}). Last checked ${e.asOf}. Data CC BY 4.0, attribute "Nuclide (nuclide.cc)".`, "", "## TL;DR", "", e.tldr, "", "## Summary", "", ...paragraphs(e.summary).flatMap((p) => [p, ""])];
...
export function registerResources(server: McpServer) {
  for (const k of KINDS) {
    server.registerResource(`kind-${k}`, `nuclide://kinds/${k}`, { title: `Nuclide ${KIND_META[k].plural}`, description: `${KIND_META[k].blurb} (${g.kind(k).length} records: id, name, status, tldr, url)`, mimeType: "application/json" },
  ...
  server.registerResource("entity", new ResourceTemplate("nuclide://entity/{id}", {
    list: async () => ({ resources: g.entities.map((e) => ({ uri: `nuclide://entity/${e.id}`, name: e.name, description: `${KIND_META[e.kind].label}: ${e.tldr}`, mimeType: "application/json" })) }),
    complete: { id: completeId },
  }), { title: "Nuclide record (JSON)", description: "Full record by id, with neighbours grouped by kind and the page URL.", mimeType: "application/json" },
  async (uri, { id }) => {
  ...
  server.registerResource("context", new ResourceTemplate("nuclide://context/{id}", { list: undefined, complete: { id: completeId } }),
    { title: "Nuclide record (Markdown context)", description: "Clean Markdown for one record: TL;DR, summary, sources, connected records. Same content as https://nuclide.cc/api/v1/context/<id>.md.", mimeType: "text/markdown" },
    async (uri, { id }) => {
      const e = g.get(String(id));
      if (!e) throw new Error(`Unknown id "${id}"`);

 ...
 * the site makes from the browser) for recruiting studies near a place, and enriches hits with Nuclide records
...
export function registerTrialsNear(server: McpServer) {
  ...
  server.registerTool("trials_near", {
    ...
    try {
      const r = await fetch(api, { headers: { Accept: "application/json", "User-Agent": "Nuclide MCP (https://github.com/casadesante/Nuclide)" } });
      ...
      return text({ query: { condition, intervention, country, center }, searchUrl: buildGeoSearchUrl({ condition, intervention, country }), studies: studies.map((s) => ({ ...s, sites: s.sites.length, nuclide: byNct.has(s.nctId) ? brief(byNct.get(s.nctId)!) : undefined })) });
    } catch (e) {

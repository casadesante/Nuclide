...
export function registerLookup(server: McpServer) {
  server.registerTool("search", {
    ...
    description: "Find Nuclide records by name or meaning. Runs a word search (names, aliases, TL;DRs, tags) and a concept search (full text plus the names of linked records) and fuses them, so paraphrases such as 'drug for HER2-low breast cancer' work. Returns id, kind, name, tldr, url and why each matched.",
  ...
  server.registerTool("ask", {
    ...
    description: "Answer a question using only sentences copied from Nuclide records. Retrieves the best records, extracts the sentences that answer the question, and returns them with a numbered citation each plus the source URLs. Nothing is generated or inferred; if the corpus has no sentence on the point it says so. Use this before summarising anything about oncology so every claim carries an Nuclide URL.",
  ...
  server.registerTool("for_cancer", {
    ...
    inputSchema: { indicationId: z.string().min(1) },
  }, async ({ indicationId }) => {
    const c = g.get(indicationId);
    if (!c || c.kind !== "indication") return fail(`Unknown cancer "${indicationId}". Cancers: ${g.kind("indication").map((x) => x.id).join(", ")}`);
    const rel: Record<string, Array<{ id: string; name: string; status?: string; url: string }>> = {};
    for (const [k, list] of g.forCancer(indicationId)) rel[k] = list.map((x) => ({ id: x.id, name: x.name, status: x.status, url: url(x) }));
    return text({ indication: { id: c.id, name: c.name, tldr: c.tldr, url: url(c), stateOfArt: c.stateOfArt, standardOfCare: c.standardOfCare, pipeline: c.pipeline, openProblems: c.openProblems }, relevant: rel });
  });

  server.registerTool("rank", {
    title: "Ranked list for a cancer and kind",
    description: "The Explore power view: rank objects of one kind (default drug) by Nuclide's disclosed relevance score, optionally for a specific cancer id. Score = standard-of-care +40, pipeline +25, history +10, direct link +15, indirect +5, plus evidence tier and connectivity. Ranks documentation and evidence, not clinical benefit.",
    inputSchema: { kind: z.enum(KINDS).default("drug"), indicationId: z.string().optional(), limit: z.number().int().min(1).max(100).default(25) },
  }, async ({ kind, indicationId, limit }) => {
    if (indicationId && !g.get(indicationId)) return fail(`Unknown cancer "${indicationId}"`);
    const list = rows.filter((r) => r.kind === kind).map((r) => ({ r, score: scoreRow(r, indicationId ?? null) })).filter((x) => x.score >= 0).sort((a, b) => b.score - a.score || a.r.name.localeCompare(b.r.name)).slice(0, limit);
    return text(list.map(({ r, score }, i) => ({ rank: i + 1, id: r.id, name: r.name, status: r.status, score, signals: indicationId ? r.rel[indicationId] : undefined, tldr: r.tldr, url: SITE_URL(r.route) })));
  });
...
const SITE_URL = (route: string) => `https://nuclide.cc${route}`;

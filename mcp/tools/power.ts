...
import { scoreRows } from "../../src/lib/biomarker-match";
...
function compareFields(e: Entity): Record<string, string | number | string[] | undefined> {
  switch (e.kind) {
    case "drug": return { brand: e.brand, code: e.code, modality: e.modality, mechanism: e.mechanism, payload: e.payload, linker: e.linker, targets: names(e.targets), indications: names(e.indications), companies: names(e.companies), firstApproval: e.approvals.length ? Math.min(...e.approvals.map((a) => a.year)) : undefined, approvals: e.approvals.map((a) => `${a.region} ${a.year}: ${a.indication}`), dosing: e.dosing ? `${e.dosing.route}; ${e.dosing.schedule}` : undefined, grade3Toxicities: e.toxicity.filter((t) => t.grade3PlusPct !== undefined).map((t) => `${t.event} ${t.grade3PlusPct}%`), keyTrials: names(e.trials) };
    case "technology": return { principle: e.principle, generation: e.generation, since: e.since, strengths: e.strengths, limitations: e.limitations, indications: names(e.indications), drugs: names(e.drugs), targets: names(e.targets) };
    case "target": return { symbol: e.symbol, targetClass: e.targetClass, biology: e.biology, whereFound: e.whereFound, drugs: names(e.drugs), indications: names(e.indications), pathways: names(e.pathways), prevalence: e.prevalence.map((p) => `${g.must(p.indicationId).name}: ${p.pct}${typeof p.pct === "number" ? "%" : ""}`) };
    case "trial": return { nct: e.nct, phase: e.phase, setting: e.setting, sponsor: e.sponsor, enrolled: e.enrolled, yearReported: e.yearReported, result: e.result, outcomes: e.outcomes.map((o) => `${o.endpoint}: ${o.arms.map((a) => `${a.name}${a.value !== undefined ? ` ${a.value}${o.unit ?? ""}` : ""}`).join(" vs ")}${o.hr !== undefined ? ` (HR ${o.hr})` : ""}`), drugs: names(e.drugs), indications: names(e.indications), replication: e.replication };
    case "indication": return { group: e.group, burden: e.burden, subtypes: e.subtypes, biomarkers: e.biomarkers, standardOfCare: e.standardOfCare.map((s) => `${s.setting}: ${s.approach}`), stateOfArt: e.stateOfArt, pipeline: names(e.pipeline), openProblems: e.openProblems };
...
export function registerPower(server: McpServer) {
  ...
  server.registerTool("biomarker_match", {
    ...
    inputSchema: { biomarkers: z.array(z.string()).min(1).describe("Biomarker ids (see description) or exact labels"), indicationId: z.string().optional(), limit: z.number().int().min(1).max(100).default(25) },
  }, async ({ biomarkers: wanted, indicationId, limit }) => {
    ...
    if (indicationId && !g.get(indicationId)) return fail(`Unknown cancer "${indicationId}"`);
    const scored = scoreRows(matchable, selected, indicationId ?? null).slice(0, limit);
    return text({ biomarkers: selected.map((b) => ({ id: b.id, label: b.label, note: b.note })), matches: scored.map((s) => ({ id: s.row.id, kind: s.row.kind, name: s.row.name, status: s.row.status, meta: s.row.meta, score: s.score, matchedBiomarkers: s.hits, caution: s.row.pair?.caution || undefined, url: `https://nuclide.cc${s.row.route}` })) });
  });
  ...
  server.registerTool("compare", {
    ...
    return text({ kind: es[0].kind, columns: cols, differingFields: differs, compareUrl: `https://nuclide.cc/compare/?ids=${ids.join(",")}` });
  });

  server.registerTool("regimens", {
    title: "Standard-of-care regimens for a cancer",
    description: "The standard-of-care rows for a indication: setting, approach, the products and trials each cites (with URLs), and guideline mapping (NCCN category, ESMO-MCBS) where recorded. Optionally filter by a setting keyword such as 'first line', 'adjuvant' or 'relapsed'.",
    inputSchema: { indicationId: z.string().min(1), setting: z.string().optional() },
  }, async ({ indicationId, setting }) => {
    const c = g.get(indicationId);
    if (!c || c.kind !== "indication") return fail(`Unknown cancer "${indicationId}"`);
    ...
    return text({ indication: brief(c), asOf: c.asOf, regimens: rowsOut });
  });

  server.registerTool("calendar", {
    title: "Readout and decision calendar",
    description: "Upcoming and past dated events: PDUFA dates, advisory committees, expected readouts, congresses and policy dates, each with the Nuclide records it concerns and a source where confirmed. Filter by date range (YYYY-MM-DD, YYYY-MM or YYYY-Qn), kind, or a record id.",
  ...
  server.registerTool("toxicity_compare", {
    ...
    return text({ families: [...families].sort(), products: table, compareUrl: "https://nuclide.cc/toxicity/" });
  });

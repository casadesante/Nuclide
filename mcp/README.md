...
An MCP (Model Context Protocol) server that gives AI assistants structured, cited access to [Nuclide](https://nuclide.cc), the public knowledge graph of oncology. Everything is computed from the same data files the site is built from. The one exception is `trials_near`, which queries ClinicalTrials.gov live.

Data is CC BY 4.0: name Nuclide and link to https://nuclide.cc wherever the output is used.
...
```sh
git clone https://github.com/casadesante/Nuclide.git
cd Nuclide && npm ci
npm run mcp          # stdio transport
```

Or `node mcp/bin.mjs`. For a zero-install route that needs no checkout, `packages/nuclide-mcp` publishes `npx -y nuclide-mcp`: it reads the public API and covers search, records, lists, cited answers, Markdown context and comparison (see `packages/nuclide-mcp/README.md` and `docs/ACCESS.md`). The server in this folder remains the one with biomarker matching, regimens, calendar, toxicity comparison, graph paths and live trials.
...
{
  "mcpServers": {
    "nuclide": { "command": "npm", "args": ["run", "--silent", "mcp"], "cwd": "/path/to/Nuclide" }
  }
}
```
...
```sh
claude mcp add nuclide -- npm run --silent --prefix /path/to/Nuclide mcp
```
...
{
  "mcpServers": {
    "nuclide": { "command": "npm", "args": ["run", "--silent", "--prefix", "/path/to/Nuclide", "mcp"] }
  }
}
```
...
| `trials_near` | Live ClinicalTrials.gov query for recruiting studies by condition or intervention near a country or coordinates, cross-referenced to Nuclide trial records. |

## Resources

- `nuclide://kinds/{kind}`: JSON list of every record of a kind (cancer, drug, target, trial, ...).
- `nuclide://entity/{id}`: one record as JSON with neighbours (completion on id).
- `nuclide://context/{id}`: one record as clean Markdown (TL;DR, summary, sources, connected records), the same content as `https://nuclide.cc/api/v1/context/<id>.md`.
...
Each prompt ends with the same ground rules: use only tool output, cite an Nuclide URL after every fact, never add numbers that are not in a result, and close with the note that Nuclide is an orientation tool, not medical advice.
...
- Scores in `rank` and `biomarker_match` are the disclosed formulas from the site (see https://nuclide.cc/about/); they rank documentation and evidence, not clinical benefit.
